...
class Ijen extends CI_Model {
	...
	public function simpanjenbaru($dtnotrans) {
		...
		$datasimpan = array(
			...
			'kode_unitR' => $this->input->get("kdunit"), 
			'nama_unitR' => $this->input->get("unit"), 
	...
	public function unitinstalasikamarjenazah() {
		$this->db->from("munit");
		$this->db->where("kode_unit", "JENA");
		$this->db->limit(1);
		$data = $this->db->get();
		return $data->row();
	}
	...
	public function datatindakaninstalasi() {
		$id = $this->input->get("id");
		$dt = explode("_", $id);
		$this->db->select("mtindakan.tindakan as tindakan, mtindakan.jasas as jasas, ptindakan_forensik.id as id, ptindakan_forensik.nonasuransi, ptindakan_forensik.diskon, ptindakan_forensik.nama_dokter ");
		$this->db->from("ptindakan_forensik");
		$this->db->join("mtindakan", "mtindakan.kode_tindakan = ptindakan_forensik.tindakan");
		$this->db->where("ptindakan_forensik.notransaksi", $dt[1]);
		$this->db->where("ptindakan_forensik.notransaksi_IN", $dt[2]);
		$this->db->where("ptindakan_forensik.kode_unit", $this->input->get("kdunit"));
		$data = $this->db->get();
		return $data->result();
	}
	...
	public function datatindakanins() {
		$this->db->select("mtindakan.tindakan as tindakan, mtindakan.jasas as jasas, ptindakan_forensik.id as id, ptindakan_forensik.nonasuransi, ptindakan_forensik.diskon, ptindakan_forensik.nama_dokter as nama_dokter");
		$this->db->from("ptindakan_forensik");
		$this->db->join("mtindakan", "mtindakan.kode_tindakan = ptindakan_forensik.tindakan");
		$this->db->where("ptindakan_forensik.notransaksi", $this->input->get("notrans"));
		$this->db->where("ptindakan_forensik.notransaksi_IN", $this->input->get("notransin"));
		$this->db->where("ptindakan_forensik.kode_unit", $this->input->get("kdunit"));
		$data = $this->db->get();
		return $data->result();
	}
	...
	public function simpantindakanjen($dtpasien, $dthargatindakan, $ambilnotrans) {
		...
		$date = date_create($this->input->get("tgljen"));
		$tgl = date_format($date,"Y-m-d");
		$datasimpan = array(
			'kode_unit' => $this->input->get("kdunit"),
			'nama_unit' => $this->input->get("unit"),
			...
			'kode_dokter' => $this->input->get("kddokter"),
			'nama_dokter' => $this->input->get("nmdokter"), 
			'kode_perawat1' => $this->input->get("kdperawat1"), 
			'nama_perawat1' => $this->input->get("nmperawat1"), 
			'kode_perawat2' => $this->input->get("kdperawat2"), 
			'nama_perawat2' => $this->input->get("nmperawat2"), 
			'kode_perawat3' => $this->input->get("kdperawat3"), 
			'nama_perawat3' => $this->input->get("nmperawat3"), 
		...
		$dt = $this->db->insert('ptindakan_forensik', $datasimpan);
		return $dt;
	}

	public function ambileditdatatindakan() {
		$id = $this->input->get("id");
		$this->db->select("mtindakan.tindakan as tindakan, mtindakan.kode_tindakan as kode_tindakan, mtindakan.jasas as jasas, ptindakan_forensik.id as id,ptindakan_forensik.nonasuransi as nonasuransi,ptindakan_forensik.diskon as diskon, ptindakan_forensik.kode_dokter as kode_dokter, ptindakan_forensik.nama_dokter as nama_dokter, ptindakan_forensik.kode_perawat1, ptindakan_forensik.nama_perawat1, ptindakan_forensik.kode_perawat2, ptindakan_forensik.nama_perawat2, ptindakan_forensik.kode_perawat3, ptindakan_forensik.nama_perawat3 ");
		$this->db->from("ptindakan_forensik");
		$this->db->join("mtindakan", "mtindakan.kode_tindakan = ptindakan_forensik.tindakan");
		$this->db->where("ptindakan_forensik.id", $id);
		$data = $this->db->get();
		return $data->row();
	}
	...
	public function ubahpelayanantindakan($dthargatindakan) {
		...
		$dt = $this->db->update('ptindakan_forensik', $datasimpan);
		return $dt;
	}
	...
	public function hapuspelayanantindakan() {
		$id = $this->input->get("id");
		$this->db->where('id', $id);
		$dt = $this->db->delete('ptindakan_forensik');
		return $dt;
	}
...
*/
    ...
    public function ubahjen() {
		...
		// $date1 = date_create($this->input->get("tgl_ambil"));
		// $tgl_ambil = date_format($date1,"Y-m-d");

		if (trim($this->input->get("tgl_ambil") == "")) {
			$tgl_ambil =null; 
		} else {		
			$date1 = date_create($this->input->get("tgl_ambil"));
			$tgl_ambil = date_format($date1,"Y-m-d");
		}
		if (trim($this->input->get("tgl_bayar") == "")) {
			$tgl_bayar =null; 
		} else {		
		$date2 = date_create($this->input->get("tgl_bayar"));
		$tgl_bayar = date_format($date2,"Y-m-d");
		}
