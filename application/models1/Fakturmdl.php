...
class Fakturmdl extends CI_Model {
    ...
    public function ambilheaderfakturbhp($id) {
    ...
    public function simpaneditdatafaktur() {
        $noterima = $this->input->get("sj");
        $date2 = date_create($this->input->get("exp"));
        $tglexp = date_format($date2,"Y-m-d");
            $detailfaktur = array(
            ...
            $this->db->where("id", $this->input->get("iddetail"));
            $dt=$this->db->update("pfakturdetail", $detailfaktur);
            return array("sukses" => $dt, "noterima" => $noterima);
    }
    ...
    public function deletedetailfaktur_old_disi_dengan_1() {
    ...
    public function deletedetailfaktur() {
        ...
        //cek apaakah ada ampra dr data dibawah ini
        $this->db->select("id");
        ...
        if ($datadetail == 0) {
            $this->db->where("id", $id);
            $dt = $this->db->delete("pfakturdetail");
            return $dt;
        } else {
            return "0";
        }
    ...
    public function ambildetailfaktur() {
        $id = $this->input->get("id");
		$this->db->from("pfakturdetail");
        $this->db->where('id', $id);
        $data = $this->db->get();
