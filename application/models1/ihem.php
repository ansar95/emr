...
class Ihem extends CI_Model {

	public function datahem() {
	...
	public function simpanhembaru($dtnotrans) {
		...
		$datasimpan = array(
			...
			'kode_radiografer' => "",
			'nama_radiografer' => "",
			'kode_hemodialis' => $this->input->get("dpetugas"),
			'nama_hemodialis' => $this->input->get("dpetugastext"),
			'kode_kelas' => "", 
			'nama_kelas' => "", 
			'notransaksi' => $dt[0], 
	...
	public function databhpinstalasi() {
		$id = $this->input->get("id");
		$dt = explode("_", $id);
		$this->db->select("tanggal, namaobat, satuanpakai, hargapakai, qty, id, nonasuransi, diskon,pakai_ulang");
		$this->db->from("pbhp");
	...
	public function simpantindakanhem($dtpasien, $dthargatindakan, $ambilnotrans) {
		$um = $this->input->get("tdumum");
		$date = date_create($this->input->get("hemtgl"));
		$tgl = date_format($date,"Y-m-d");
		$datasimpan = array(
	...
	public function ambileditdatabhp() {
		$id = $this->input->get("id");
		$this->db->select("tanggal, namaobat, satuanpakai, hargapakai, qty, id, kodeobat, nonasuransi, diskon, pakai_ulang");
		$this->db->from("pbhp");
		$this->db->where("id", $id);
		$data = $this->db->get();
		return $data->row();
...
*/
	...
	public function hapusinstalasi() {
        $id = $this->input->get("id");
        $this->db->select("register_instalasi.notransaksi_IN as notransaksi_IN, register_instalasi.notransaksi as notransaksi,register_instalasi.kode_unit as kode_unit, munit.hem as hem");
        $this->db->from("register_instalasi");
