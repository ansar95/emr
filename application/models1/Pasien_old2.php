...
class Pasien extends CI_Model {
	...
	public function ambil_antrian_poli($kdpoli) {
		$tgl = date("Y-m-d");
    ...
    public function editregisuri($hasilicd) {
		...
		$golc = trim($this->input->get("goltext"));
			...
			//cari data id record pertama pada register_detail
			$no_rm=$this->input->get("rm");
			$qry24 = $this->db->query("SELECT notransaksi from register WHERE no_rm='$no_rm' and bagian='INAP' and status=0  LIMIT 1");
			$qry24row = $qry24->row();
			$notransaksi = $qry24row->notransaksi;
	...
	public function carinamapasienurj() {
		...
		$this->db->select("register.bagian as bagian, register.id as id, register.billing as billing, pasien.title as title");
		$this->db->from("register");
		$this->db->join("register_detail", "register_detail.notransaksi = register.notransaksi");
		$this->db->join("pasien", "pasien.no_rm = register.no_rm");
	...
	public function simpanregisurj($dtnotrans, $hasilicd, $dtantrian_poli) {
	    $this->db->select("id");
	    $this->db->from("register_detail");
	    $this->db->where("status", "0");
        $this->db->where("no_rm", $this->input->get("rm"));
	...
	function fetch_pasien_detail($data) {
		...
		<tr>
			...
			<th width="7%">Tgl Keluar</th>
			<th width="7%">Jam Keluar</th>
			<th width="20%">DPJP</th>
			<th width="6%">User-1</th>
			<th width="6%">User-2</th>
			<th width="10%">LastLogin</th>
			<th width="10%">No.Transaksi</th>
		</tr>
		';
		foreach($query->result() as $row) {
	...
	function fetch_pasien_pengelolaan($limit, $start) {
		$output = '';
		$this->db->select("register_detail.id as id, register_detail.no_transaksi_secondary as no_transaksi_secondary, register_detail.no_rm as no_rm, pasien.nama_pasien as nama_pasien, register.tgl_masuk as tgl_masuk, register.jam_masuk as jam_masuk, register.asuransi as asuransi, register_detail.nama_unit as nama_unit, register_detail.nama_kelas as nama_kelas, register_detail.tgl_masuk_kamar as tgl_masuk_kamar , register_detail.jam_masuk as jam_masuk, register.id as idreg, register_detail.notransaksi as notransaksi");
		$nrm = $this->input->get("nrm");
		$kodeunit = $this->input->get("kodeunit");
		$this->db->from("pasien");
		$this->db->join("register", "register.no_rm = pasien.no_rm");
		...
		<tr>
			...
			<th width="3%"><div align="center">BAYI</div></th>
		</tr>
		';
		foreach($query->result() as $row) {
		$output .= '
		<tr>
			...
			<td><div align="center">'.$row->jam_masuk.'</div></td>
			<td class="text-center">
				<a onclick="panggilpindahkamar(\''.$row->notransaksi.'\');" class="btn-sm btn-primary btn-flat">Pindah</a>
			...
			<td class="text-center">
				<a onclick="cetakgelangbayi(\''.$row->idreg.'\')" class="btn-sm btn-success btn-flat">GL</a>
			</td>		
		</tr>
		';
		}
	...
	function count_all_pengelolaan_statusrmxx() {
		$this->db->select("register_detail.no_rm");
		$nrm = $this->input->get("nrm");
		$kodeunit = $this->input->get("kodeunit");
		$this->db->from("pasien");
	...
	function fetch_pasien_pengelolaan_statusrmxx($limit, $start) {
		$output = '';
		$nrm = $this->input->get("nrm");
		$kodeunit = $this->input->get("kodeunit");
		$tgl=date('Y-m-d');
	...
	function fetch_pasien_pengelolaan_statusrm($limit, $start) {
		...
		$kodeunit = $this->input->get("kodeunit");
		$tgl=date('Y-m-d');
		$this->db->select("register_detail.id as id, register_detail.no_transaksi_secondary as no_transaksi_secondary, register_detail.no_rm as no_rm, pasien.nama_pasien as nama_pasien, pasien.alamat as alamat, register.tgl_masuk as tgl_masuk, register.jam_masuk as jam_masuk, register.asuransi as asuransi, register_detail.nama_unit as nama_unit, register_detail.nama_kelas as nama_kelas, register_detail.tgl_masuk_kamar as tgl_masuk_kamar , register_detail.jam_masuk as jam_masuk, register_detail.pindah as pindah");
		// $this->db->select("register_de.no_rm as no_rm,pasien.nama_pasien as nama_pasien, register.bagian as bagian,  ");	
		$this->db->from("register");
		$this->db->join("pasien", "register.no_rm = pasien.no_rm");
		$this->db->join("register_detail", "register_detail.notransaksi = register.notransaksi");
		...
		<tr>
			...
			<td>'.$row->nama_unit.'</td>
			<td>'.$row->asuransi.'</td>
			<td><div align="center">'.$row->pindah.'</div></td>
			<td class="text-center">
				<a onclick="panggillihat(\''.$row->no_rm.'\')" class="btn-sm btn-warning btn-flat">History</a>
			</td>
		</tr>
	...
	function fetch_pasien_pengelolaanpulang($limit, $start) {
		...
		<tr>
			...
			<td class="text-center">
				<a onclick="panggillihat(\''.$row->no_rm.'\')" class="btn-sm btn-warning btn-flat">History</a>
				<a onclick="batalpulang(this, \''.$row->id.'\');" class="btn-sm btn-danger btn-flat">BATAL PULANG</a>
			</td>
