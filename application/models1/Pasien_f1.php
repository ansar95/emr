...
class Pasien extends CI_Model {
	...
	public function getUnitRawatJalanByDokter($kode_dokter)
	{
		...
		if ($unit_rj) {
			return $unit_rj[0]->kode_poli;
		}
		return;
	}

	public function getUnitByDokter($kode_dokter)
	{
		$this->db->from('munit');
		$this->db->where('kode_dokter',$kode_dokter);
		$this->db->limit(1);
...
// =================DISINI OTOMATIS RADIOLOGI ========
	...
	function fetch_pasien_pengelolaan($limit, $start) {
		...
		<tr>
			...
			<td><div align="center">'.$row->jam_masuk.'</div></td>
			<td class="text-center">
				<button onclick="panggilpindahkamar(\''.$row->notransaksi.'\');" class="btn btn-sm btn-primary m-1">Pindah</button>
				<button onclick="pasienkembali(this, \''.$row->id.'\');" class="btn btn-sm btn-danger m-1">Kembali</button>
				<button onclick="panggillihat(\''.$row->no_rm.'\')" class="btn btn-sm btn-warning m-1">History</button>
			</td>
			<td class="text-center">
