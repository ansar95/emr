...
class Urjmdl extends CI_Model {
	...
	public function dtlistresep($irm) {
		$no = $this->input->get("notrans");
		$unit = $this->input->get("unit");
		$this->db->select("resep_header.idnoresep, resep_header.tglresep, resep_header.nama_dokter, resep_header.nama_unit, resep_header.noresep ");
		$this->db->from("resep_header");
		$this->db->where("resep_header.no_rm", $irm);
...
//		$this->db->where("register_detail.kode_unit", $unit);
	...
	public function datainst_lab($rm) {
		// $no = $this->input->get("notrans");
		// $unit = $this->input->get("unit");
		$this->db->select("id, tanggal, nama_unit, nama_dokter, nama_unitR, nama_dokter_periksa, notransaksi_IN, catatan, kode_unit");
		$this->db->from("register_instalasi");
	...
	public function datainst_rad($rm) {
		$this->db->select("ptindakan_instalasi.id, ptindakan_instalasi.tanggal,mtindakan.tindakan,ptindakan_instalasi.notransaksi_IN, ptindakan_instalasi.notransaksi");
		...
		$this->db->or_where("ptindakan_instalasi.kode_unit", 'RDGL');
		$this->db->where("ptindakan_instalasi.no_rm", $rm);
        $this->db->order_by("ptindakan_instalasi.id", "desc");
		$data = $this->db->get();
		return $data->result();
	}
	//
	...
	public function simpanpindahkamar($dtregis) {
		...
		//-- taskid 5
		//cek dulu apakah sdh ada taskid5
...
// tambahan untuk resep
	...
	public function simpanantrian() {
		...
		$qry22=$this->db->query("update register set taskid4=1,jam_panggil='$jam_sekarang' where kode_booking='$kodebooking' limit 1");
	...
	public function simpantindakanlab() {
		...
		//hapus dulu sebelum menulis ulang
        $this->db->delete('ptindakan_instalasi',array('notransaksi_IN '=>$notransin,'no_rm'=>$rm,'kode_unit'=>'LABS','notransaksi'=>$notrans));
	...
	public function simpantindakanrad() {
		...
		//hapus dulu sebelum menulis ulang, perlu validasi untuk cek dulu apakah ada hasil atau tidak
        $this->db->delete('ptindakan_instalasi',array('notransaksi_IN '=>$notransin,'no_rm'=>$rm,'kode_unit'=>'RDGL','notransaksi'=>$notrans));
		...
		public function berkastibadipoli() {
			...
			$qry22=$this->db->query("update register set berkas=2,jam_terima='$jam_sekarang' where id='$idreg' limit 1");
		...
		public function ambildatadiagsoap() {
		...
		public function ambildiagpasien($notran) {
			$this->db->from("pdiagnosa");
			$this->db->where("notransaksi", $notran);
			$data = $this->db->get();
			// return $data->result();
			// return $data;
			return $data->row();
		...
		public function savedatasoap() {
			$id=$this->input->get("id");
			// alertWindow($this->input->get("nadi"));
			$datasimpan = array(
				...
				'diagnosatext'=>$this->input->get('diagnosatext'),
				'konsuldokter'=>$this->input->get('konsuldokter')
			);
			$this->db->where("id",$id);
			$this->db->limit(1);
	...
	public function savedataassesmen() {
		//cek dulu aoakah ada record untuk 
		$id=$this->input->get("id");
		$no_rm=$this->input->get("no_rm");
		$pelayanan='RJ';
		$keluhanUtama=$this->input->get("keluhanUtama");
		$penyakitSekarang=$this->input->get("penyakitSekarang");
		$penyakitDahulu=$this->input->get("penyakitDahulu");
		$riwayatAlergi=$this->input->get("riwayatAlergi");
		$riwayatAlergiText=$this->input->get("riwayatAlergiText");

		$kesadaran=$this->input->get("kesadaran");
		$kesadaranText=$this->input->get("kesadaranText");
		$kesadaranUmum=$this->input->get("kesadaranUmum");
		$ttvTD=$this->input->get("ttvTD");
		$ttvTDteratur=$this->input->get("ttvTDteratur");
		$ttvTDtidakTeratur=$this->input->get("ttvTDtidakTeratur");

		$ttvRR=$this->input->get("ttvRR");
		$ttvSuhu=$this->input->get("ttvSuhu");
		
		alertWindow($this->input->get("nadi"));
		$datasimpan = array(
			
		// id: id,
		// assKeluhanutama : assKeluhanutama,
		// assRiwayatsekarang : assRiwayatsekarang,
		// assalergiTidak : assalergiTidak,
		// assalergiYa : assalergiYa,
		// assRiwayatdahulu : assRiwayatdahulu,
		// assAlergi : assAlergi,
		// assKesadaran : assKesadaran,
		// assKesadaranlainnya : assKesadaranlainnya,
		// assKesadaranumum : assKesadaranumum,
		// assSuhu : assSuhu,
		// assTinggi : assTinggi,
		// aasBerat : aasBerat,
		// assTensi : assTensi,
		// aasNadi : aasNadi,
		// assRespirasi : assRespirasi,
		// assSpo2 : assSpo2,
		// assGcs : assGcs,
		...
		$this->db->limit(1);
		$dt = $this->db->update('psoap', $datasimpan);
		//simpan soap di tabel pdiagnosa
		//cek dulu apakah ada data sebelumnya
		// $qry23=$this->db->query("select psoap.notransaksi as notransaksi,register.tgl_masuk as tgl_masuk, register.no_rm as no_rm from psoap,register where psoap.notransaksi=register.notransaksi and psoap.id='$id' limit 1");
		// foreach ($qry23->result_array() as $brs9) {
		// 	$notransaksi=$brs9['notransaksi'];
		// 	$tgl_masuk=$brs9['tgl_masuk'];
		// 	$no_rm=$brs9['no_rm'];
		// }
		// $user1 =$this->session->userdata("nmuser");
		// $user2 = $this->session->userdata("nmuser"); 
		// $lastlogin = date("Y-m-d H:i:s"); 
		// $codeicd=trim($this->input->get('diagnosa'));

		// $qry23=$this->db->query("select * from micd1 where icd_code='$codeicd' limit 1");
		// foreach ($qry23->result_array() as $brs91) {
		// 	$nodaftar=$brs91['icd_code'];
		// 	$nodtd=$brs91['dtd'];
		// 	$icdlatin=$brs91['jenis_penyakit_local'];
		// 	$diagnosa=$brs91['jenis_penyakit'];
		// 	$type=1;
		// }
		// $qry24=$this->db->query("select count(id) as jumid from pdiagnosa where notransaksi='$notransaksi' and type=1 limit 1");
		// foreach ($qry24->result_array() as $brs9) {
		// 	$jumid=$brs9['jumid'];
		// }
		// if ($jumid > 0) {
		// 	// update
		// 	$this->db->query("update pdiagnosa set diagnosa='$diagnosa', nodaftar='$nodaftar', nodtd='$nodtd', icdlatin='$icdlatin', user2='$user2' where notransaksi='$notransaksi' and type=1 limit 1");
		// } else {
		// 	// insert
		// 	$this->db->query("insert into pdiagnosa (no_rm,tgl_masuk,type,diagnosa,nodaftar,nodtd,user1,user2,lastlogin,notransaksi,icdlatin) VALUES ('$no_rm','$tgl_masuk','$type','$diagnosa','$nodaftar','$nodtd','$user1','$user2','$lastlogin','$notransaksi','$icdlatin')" );
		// }
}

	public function cekdiagnosaada() {
		$id = $this->input->get("id");
		$this->db->select("notransaksi");
		$this->db->from("register_detail");
		$this->db->where('id', $id);
		...
		$this->db->limit(1);
		$dt = $this->db->get();	
		$jumlahbaris=$dt->num_rows();

		// return $jumlahbaris; //uji diagnosa di isi atau belum
		return 1;
	}
}
