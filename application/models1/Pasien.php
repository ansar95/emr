...
class Pasien extends CI_Model {
	...
	public function ambilpasiendariregis_rjalan() {
        $id = $this->input->get("id");
	...
	public function ambilnosep() {
        $id = $this->input->get("id");
        $this->db->from('register_detail');
        $this->db->select('register.nosep');
	...
	public function ambilnosep_igd() {
        $nosep = $this->input->get("nosep1");
        $this->db->from('register');
        $this->db->select('nosep_igd');
	...
	public function ambiljumlahbaris() {
        $id = $this->input->get("id");
		$query = $this->db->query("SELECT no_rm, notransaksi, nosep FROM register where id='$id' and nosep<>'' ");
	...
	public function ambiljumlahbaris_ranap() {
        $id = $this->input->get("id");
		$query = $this->db->query("SELECT no_rm, notransaksi, nosep FROM register where id='$id' and nosep<>'' and nosep_igd<>'' ");
		$total = $query->num_rows();	
		if ($total > 0) {
			return 1;
		} else {
	...
	public function ambiljumlahbaris_spri() {
        $id = $this->input->get("id");
		$query = $this->db->query("SELECT register.no_rm, register.notransaksi, register.nosep, spri_resources.nospri FROM register,spri_resources where register.notransaksi=spri_resources.notransaksi and register.id='$id' ");
		$total = $query->num_rows();	
		if ($total > 0) {
			return 1;
		} else {
	...
	public function ambiljumlahbaris_surkon() {
        $id = $this->input->get("id");
		$query = $this->db->query("SELECT register.no_rm, register.notransaksi, register.nosep, spri_resources.nospri FROM register,spri_resources where register.notransaksi=spri_resources.notransaksi and register.id='$id' ");
		$total = $query->num_rows();	
		if ($total > 0) {
	...
	public function ambildataregistrx() {
        $id = $this->input->get("id");
		$query = $this->db->query("SELECT no_rm, notransaksi, nosep, nosep_igd FROM register where id='$id' ");
		$total = $query->num_rows();	
		if ($total != 0) {
			$datahasil = $query->row();
			return $datahasil;
	...
	public function ambildataregistrx_spri() {
        $id = $this->input->get("id");
		$query = $this->db->query("SELECT register.no_rm, register.notransaksi, register.nosep, spri_resources.nospri FROM register,spri_resources where register.notransaksi=spri_resources.notransaksi and register.id='$id' ");
		$total = $query->num_rows();	
		if ($total != 0) {
	...
	public function ambildataregistrx_langsung() {
        $id = $this->input->get("id");
        $this->db->from('register');
        $this->db->select('no_rm, notransaksi, nosep');
        $this->db->where('id', $id);
        $datano = $this->db->get();
		$hasil = $datano->num_rows();
		$datahasil = $datano->row();
		return $datahasil;
    }
	...
	public function carinamapasienugd() {
		...
		$this->db->select("register_detail.nama_unit as nama_unit, register.billing as billing, pasien.title as title, register.nosep as nosep");
		$this->db->from("register");
	...
	public function ambildataregisdetail() {
        $id = $this->input->get('id');
        $this->db->from('register_detail');
        $this->db->where('id', $id);
        $datano = $this->db->get();
	...
	public function carinamapasienuri() {
		...
		$this->db->select("register.no_rm as no_rm, pasien.nama_pasien as nama_pasien, pasien.alamat as alamat, register.nmkelas as nmkelas, register.tgl_masuk as tgl_masuk");
		$this->db->select("register.rujukan as rujukan, register.golongan as golongan, register.notransaksi as notransaksi, register.bagian as bagian, register.id as id");
		$this->db->select("register_detail.nama_unit as nama_unit, register.billing as billing, register_detail.id as iddetail, pasien.title as title, register.nosep as nosep ");
		$this->db->from("register");
		$this->db->join("pasien", "pasien.no_rm = register.no_rm");
        $this->db->join("register_detail", "register_detail.notransaksi = register.notransaksi");
	...
	public function carinamapasienurj() {
		...
		$this->db->select("register.tgl_masuk as tgl_masuk, register.rujukan as rujukan, register.golongan as golongan, register.notransaksi as notransaksi, register.nosep as nosep ");
...
//ada backup di bagian atas
	...
	public function carinama() {
        $noka = $this->input->get("noka");
        $this->db->from('pasien');
        $this->db->select('nama_pasien');
        $this->db->where('no_askes', $noka);
        $datano = $this->db->get();
        $no = $datano->row();
        return $no->nama_pasien;
    }

	public function caridatapulang() {
        $notrx = $this->input->get("notrx");
		$this->db->select("register.no_rm as no_rm, register.nosep as nosep, pasien.no_askes as noka, pasien.nama_pasien as nama_pasien, register.statuspulangvclaim, register.tglmeninggal, register.suratmeninggal as nosurat, register.tglpulangvclaim as tglpulang");
        $this->db->from("register");
        $this->db->join("pasien", "register.no_rm = pasien.no_rm");
        $this->db->where("register.notransaksi", $notrx);
        $data = $this->db->get();
        return $data->row();
    }
	...
	public function caridatarujukan() {
        $notrx = $this->input->get("notrx");
		$this->db->select("register.no_rm as no_rm, register.nosep as nosepreg, pasien.no_askes as noka, pasien.nama_pasien as nama_pasien, vclaim_rujukan.*");
        $this->db->from("register");
        $this->db->join("pasien", "register.no_rm = pasien.no_rm");
        $this->db->join("vclaim_rujukan", "register.notransaksi = vclaim_rujukan.notransaksi","left");
        $this->db->where("register.notransaksi", $notrx);
        $data = $this->db->get();
        return $data->row();
    }
