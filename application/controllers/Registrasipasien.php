...
//pulangkan pasien
	...
	public function formsepigdedit_old()
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
	...
	public function formsepigdedit()
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$cekdataada = $this->pasien->ambiljumlahbaris();
			if ($cekdataada > 0) {
				$dtnorm = $this->pasien->ambildataregistrx();
				$dtnotransaksi = $dtnorm->notransaksi;
				$this->load->model("bpjsmodel");
	...
	public function formsepirj()
	{
		if ($this->session->userdata("login") == TRUE) {
			...
			$dtregis = $this->pasien->ambildataregisugd();
			$notransaksi=$dtregis->notransaksi;
			$norujukan=$dtregis->norujukan;
			$faskes=$dtregis->ppkrujukan;
			// alertWindow('1   '.$faskes);
			$dtnorm = $this->pasien->ambilpasiendariregis();
			$dtpasien = $this->pasien->pasiendatarm($dtnorm);
			$hp=$dtpasien->hp;
	...
	public function formhapusirj_int()
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$cekdataada = $this->pasien->ambiljumlahbaris();
	...
	public function formsepiri()
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$cekdataada = $this->pasien->ambiljumlahbaris_spri();
			if ($cekdataada > 0) {
				$dtnorm = $this->pasien->ambilpasiendariregis();
				$dtpasien = $this->pasien->pasiendatarm($dtnorm);
				$hp=$dtpasien->hp;
	...
	public function formsepiriedit()  
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$cekdataada = $this->pasien->ambiljumlahbaris_ranap();
			if ($cekdataada > 0) {
				$dtnorm = $this->pasien->ambildataregistrx();
				$dtnotransaksi = $dtnorm->notransaksi;
				$this->load->model("bpjsmodel");
				$dtsep=$this->bpjsmodel->ambildatasep_rinap($dtnotransaksi);  
				$dtspri=$this->bpjsmodel->ambildataspri($dtnotransaksi);
				$kelashak=$dtsep->peserta_hak_kelas;
				if ($kelashak=='KELAS I') {
					$nokelas='1';
				} else if ($kelashak=='KELAS II')  {
	...
	public function formsepirihapus()  
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$cekdataada = $this->pasien->ambiljumlahbaris_ranap();
	...
	public function formspriedit()
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$cekdataada = $this->pasien->ambiljumlahbaris_spri();
			if ($cekdataada > 0) {
				$dtnorm = $this->pasien->ambildataregistrx_spri();
				$dtnotransaksi = $dtnorm->notransaksi;
				$this->load->model("bpjsmodel");
				$dtspri=$this->bpjsmodel->ambildataspri($dtnotransaksi);
				...
				$data["dtspri"] = $dtspri;
				$data["dtnorm"] = $dtnorm;
				$data["notransaksi"] = $dtnotransaksi;
				$data["formpilih"] = 1;
				$this->load->view('layanan/pelayanan/bpjs/form/formspri', $data);
			} else {
				$data["formpilih"] = 3;
	...
	public function formsurkonedit()
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$cekdataada = $this->pasien->ambiljumlahbaris_spri();
			if ($cekdataada > 0) {
				...
				$data["notransaksi"] = $dtnotransaksi;
				$data["formpilih"] = 1;
				$this->load->view('layanan/pelayanan/bpjs/form/formspri_surkon', $data);
			} else {
				$data["formpilih"] = 3;
				$this->load->view('layanan/pelayanan/bpjs/form/formspri_surkon', $data);
			}	
	...
	public function formsurkon()
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$dtnorm = $this->pasien->ambilpasiendariregis_rjalan();
			$dtpasien = $this->pasien->pasiendatarm($dtnorm);
			$nosep = $this->pasien->ambilnosep();
			$hp=$dtpasien->hp;
			$no_askes=$dtpasien->no_askes;
			// $dthistory = $this->pasien->ambilhistoryregistrasi($dtpasien->no_rm, "IGD");
			$dtregis = $this->pasien->ambildataregisdetail();
			$notransaksi=$dtregis->notransaksi;
			$id=$dtregis->id;
			$data["txtTanggal"] = date('Y-m-d');
			$data["txtno_rm"] = $dtnorm;
	...
	public function formsurkonhapus()
	{
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("pasien");
			$cekdataada = $this->pasien->ambiljumlahbaris_spri();
