...
class Rme extends CI_Controller {
	...
	public function rme_new() {
		if ($this->session->userdata("login") == TRUE) {
			...
			$data['kosong'] = 1;
			$tabnav = $this->input->get('tabnav'); // Mendapatkan nilai tabnav dari query string
			if ( $tabnav == NULL ) {
				$data['tabnav'] = 1;
			} else {
				$data['tabnav'] = $tabnav;
	...
	public function rme_form_pasien() {
		if ($this->session->userdata("login") == TRUE) {
			...
			// if ($editing == 1) {
				...
				$dtlabheader_semua = $this->rmemdl->panggillabheader_semua_ranap($no_rm,$notransaksi);
				$dtradheader_semua = $this->rmemdl->panggilradheader_semua_ranap($no_rm,$notransaksi);
				$data['dtlabheader_semua'] = $dtlabheader_semua;
				$data['dtradheader_semua'] = $dtradheader_semua;

				$dtlabheader_hariini = $this->rmemdl->panggillabheader_hariini_ranap($no_rm,$notransaksi);
				$dtradheader_hariini = $this->rmemdl->panggilradheader_hariini_ranap($no_rm,$notransaksi);

				$data['dtlabheader_hariini'] = $dtlabheader_hariini;
				$data['dtradheader_hariini'] = $dtradheader_hariini;
...
//kebutuhan isi tindakan instalasi
	...
	public function hapusLabRme() {
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtlab = $this->rmemdl->hapusdetaillab($idnya,$notransaksi_IN);
			//ambil kembali data lab

			$dtlabheader_hariini = $this->rmemdl->panggillabheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtlabheader_hariini'] = $dtlabheader_hariini;

			$dt["dtview"] = $this->load->view('layanan/pelayanan/rme/tdlistHeaderLab_rme', $data, TRUE);
			echo json_encode($dt);
	...
	public function hapusRadRme() {
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");;
			$dtlab = $this->rmemdl->hapusdetailrad($idnya,$notransaksi_IN);
			$dtradheader_hariini = $this->rmemdl->panggilradheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtradheader_hariini'] = $dtradheader_hariini;
			$data['notransaksi_IN'] = $notransaksi_IN;
			$dt["dtview"] = $this->load->view('layanan/pelayanan/rme/tdlistHeaderRad_rme', $data, TRUE);
			echo json_encode($dt);
...
public function rme_form_pasien_ranap() {
	if ($this->session->userdata("login") == TRUE) {
		...
		$dtobatoral = $this->rmeigdmdl->panggilobatoral($no_rm,$notransaksi);
		$data['dtobatoral'] = $dtobatoral;
...
// =============gambar=================
	...
	public function updateData() {
		...
		$this->rmeigdmdl->updateDataobatrekon($id, $tindaklanjut, $perubahan, $rute, $qty);
	...
	public function cetakResumeRanap($no_rm,$notransaksi,$kode_dokter,$kode_unit) {
        if ($this->session->userdata("login") == TRUE) {
	...
	// public function cetakResumeIgd($no_rm,$notransaksi,$kode_dokter) { di gabung saja ke ranap
    //     if ($this->session->userdata("login") == TRUE) {
    ...
    //         // $this->pdf->render();
    //         // $this->pdf->output();
    //         $this->load->view('layanan/pelayanan/rme/resume/resume_igd', $data);
	...
	public function refreshFileLab() {
		$no_rm = $this->input->get('no_rm');
		$this->load->model("rmemdl");
		$dthasilinstalasiLAB = $this->rmemdl->panggilhisinstalasiLAB($no_rm);
		$data['dthasilinstalasiLAB'] = $dthasilinstalasiLAB;
		$dt["dtview"] = $this->load->view('layanan/pelayanan/rme/tdlisthistoryLAB', $data, TRUE);
		echo json_encode($dt);
	}	

	public function refreshFileRad() {
		$no_rm = $this->input->get('no_rm');
		$this->load->model("rmemdl");
		$dthasilinstalasiRAD = $this->rmemdl->panggilhisinstalasiRAD($no_rm);
		$data['dthasilinstalasiRAD'] = $dthasilinstalasiRAD;
		$dt["dtview"] = $this->load->view('layanan/pelayanan/rme/tdlisthistoryRAD', $data, TRUE);
		echo json_encode($dt);
	}	


	public function panggilRiwayat() {
		if ($this->session->userdata("login") == TRUE) {
			$no_rm = $this->input->get('no_rm');
			$notransaksi = $this->input->get('notransaksi');
			$kode_dokter = $this->input->get('kode_dokter');
			$this->load->model("rmemdl");
			$dtpsoap = $this->rmemdl->panggilRiwayatResume($no_rm,$notransaksi,$kode_dokter);
			$data['dtpsoap'] = $dtpsoap;
			echo json_encode($data);
		} else {
			redirect('login');
		}
	...
	public function panggilTerapiResep() {
		if ($this->session->userdata("login") == TRUE) {
			$no_rm = $this->input->get('no_rm');
			$notransaksi = $this->input->get('notransaksi');
			$kode_dokter = $this->input->get('kode_dokter');
			$this->load->model("rmemdl");
			$dtresepresume = $this->rmemdl->panggilresepheader_resume($no_rm,$notransaksi,$kode_dokter);
			$data['dtresepresume'] = $dtresepresume;
			echo json_encode($data);
		} else {
			redirect('login');
		}
	...
	public function cetakSuratKonsul($id) {
        if ($this->session->userdata("login") == TRUE) {
            
            $r = date("Ymd");
			$this->load->model("rmemdl");
			$dtkonsulranap = $this->rmemdl->panggilLembarKonsulRanap($id);

			// $data['dtdiagnosaresume'] = $dtdiagnosaresume;
			// $data['no_rm'] = $no_rm;
			// $data['notransaksi'] = $notransaksi;
			$data['id'] = $id;
			$data['dtkonsulranap'] = $dtkonsulranap;

            $this->load->library('pdf');
            $this->pdf->setPaper('F4', 'potrait');
            $this->pdf->filename = "resume".$id.".pdf";
            $this->pdf->load_view('layanan/pelayanan/rme/resume/format_surat_konsul', $data);
            $this->pdf->render();
            $this->pdf->output();
            // $this->load->view('layanan/pelayanan/rme/resume/format_surat_konsul', $data);
	...
	public function cetakLaporanOperasi($no_rm,$notransaksi,$notransaksi_IN) {
        if ($this->session->userdata("login") == TRUE) {
            
            $r = date("Ymd");
			$this->load->model("rmeigdmdl");
			$dtlapoperasi = $this->rmeigdmdl->panggilcetakLaporanOperasi($notransaksi_IN);

			$data['dtlapoperasi'] = $dtlapoperasi;
			$data['no_rm'] = $no_rm;
			$data['notransaksi'] = $notransaksi;
			$data['notransaksi_IN'] = $notransaksi_IN;
			$data['dtlapoperasi'] = $dtlapoperasi;
            ...
            $this->pdf->setPaper('F4', 'potrait');
            $this->pdf->filename = "resume".$no_rm.".pdf";
            $this->pdf->load_view('layanan/pelayanan/rme/resume/format_laporan_operasi', $data);
            $this->pdf->render();
            $this->pdf->output();
            // $this->load->view('layanan/pelayanan/rme/resume/format_laporan_operasi', $data);
	...
	public function saveJatuhAnak() {
		if ($this->session->userdata("login") == TRUE) {
			$no_rm = $this->input->get('no_rm');
			$kode_dokter = $this->input->get('kode_dokter');
			$notransaksi = $this->input->get('notransaksi');
			$this->load->model("rmeigdmdl");
			$dtaskepigd = $this->rmeigdmdl->simpanJatuhAnak();
			$dt["dtaskepigd"] = $dtaskepigd;
			echo json_encode($dt);
		} else {
			redirect('login');
	...
	public function addrecordobatoral() {
		if ($this->session->userdata("login") == TRUE) {
			$no_rm = $this->input->get('no_rm');
			$notransaksi = $this->input->get('notransaksi');
			$this->load->model("rmeigdmdl");
			$dtaddrecord = $this->rmeigdmdl->addrecordobatoral($no_rm,$notransaksi);

			$dtobatoral = $this->rmeigdmdl->panggilobatoral($no_rm,$notransaksi);
			$data['dtobatoral'] = $dtobatoral;
	
			$dt["dtview"] = $this->load->view('layanan/pelayanan/rmeigd/tdlistobatoral', $data, TRUE);
			echo json_encode($dt);
		} else {
			redirect('login');
		}
	...
	public function panggilformobatoral() {
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmeigdmdl");
			$dtobatoralid = $this->rmeigdmdl->dataobatoralPerId();
			// $data["no_rm"] = $no_rm;
			// $data["notransaksi"] = $notransaksi;
			$data["dtobatoralid"] = $dtobatoralid;
			$this->load->view('layanan/pelayanan/rmeigd/formobatoral', $data);
		} else {
			redirect('login');
		}
	}

	public function simpandataobatoral() {
		if ($this->session->userdata("login") == TRUE) {
			$id = $this->input->get('id');
			$no_rm = $this->input->get('no_rm');
			$notransaksi = $this->input->get('notransaksi');
			$this->load->model("rmeigdmdl");
			$simpandatatindakan = $this->rmeigdmdl->simpandataobatoral($id);
	...
	public function hapusdataobatoral() {
		if ($this->session->userdata("login") == TRUE) {
			$id = $this->input->get('id');
			$no_rm = $this->input->get('no_rm');
			$notransaksi = $this->input->get('notransaksi');
			$this->load->model("rmeigdmdl");
			$simpandatatindakan = $this->rmeigdmdl->hapusdataobatoral($id);
	...
	public function FormUpload() {
		if ($this->session->userdata("login") == TRUE) {
			...
			$this->load->view('layanan/pelayanan/rmeigd/formUpload', $data);
		} else {
			redirect('login');
		}
	}

	public function hapusFileUpload() {
		$id = $this->input->get('id');
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
	if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmeigdmdl");
            $this->rmeigdmdl->hapusfileupload($id);
            $dtfile = $this->rmeigdmdl->panggilDataUpload($no_rm,$notransaksi);
			$data['dtfile'] = $dtfile;
			$data['dtno_rm'] = $no_rm;
			$data['dtnotransaksi'] = $notransaksi;
	...
	public function tambahOrderLabHeader() { //order header lab
		if ($this->session->userdata("login") == TRUE) {
            $this->load->model("dokter");
            $data['dtDPJP'] = $this->dokter->datadokter();
            $data['kode_dokter_cek'] = $this->input->get('kode_dokter');
            $data['notransaksi'] = $this->input->get('notransaksi');
            $data['kode_unit'] = $this->input->get('kode_unit');
            $data['rirj'] = $this->input->get('rirj');
            $data['tgl_masuk'] = $this->input->get('tgl_masuk');
			$this->load->view('layanan/pelayanan/rme/instalasi/formOrderLab', $data);
		} else {
			redirect('login');
		}
	}

	public function saveOrderHeaderLab() {
		$no_rm = $this->input->get('no_rm');
		$kode_dokter = $this->input->get('kode_dokter');
		$notransaksi = $this->input->get('notransaksi');
		$rirj = $this->input->get('rirj');
		$kd='LABS';
		$nm='LABORATORIUM';
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtresep = $this->rmemdl->saveLabHeader($kd,$nm);
			$dtlabheader_hariini = $this->rmemdl->panggillabheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtlabheader_hariini'] = $dtlabheader_hariini;
	...
	public function hapusLabHeader() {
		$no_rm = $this->input->get('no_rm');
		$kode_dokter = $this->input->get('kode_dokter');
		$notransaksi = $this->input->get('notransaksi');
		$notransaksi_IN = $this->input->get('notransaksi_IN');
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtresep = $this->rmemdl->hapusLabHeader($notransaksi_IN);
			$dtlabheader_hariini = $this->rmemdl->panggillabheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtlabheader_hariini'] = $dtlabheader_hariini;
	...
	public function tambahOrderLab() { //obat baru yg sebelumnya tdk ada di histori resep pasien
		if ($this->session->userdata("login") == TRUE) {
            $this->load->model("rmemdl");
            $data['dtpemeriksaanlab'] = $this->rmemdl->ambildatatindakanlab_rme();
			$data['no_rm'] = $this->input->get('no_rm');
			$data['notransaksi'] = $this->input->get('notransaksi');
			$data['notransaksi_IN'] = $this->input->get('notransaksi_IN');
			$data['pilihlab'] = $this->input->get('pilihlab');
			$this->load->view('layanan/pelayanan/rme/formTambahPemeriksaanLab', $data);
		} else {
			redirect('login');
		}
	}

	public function saveItemLab() {
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
		$rirj = $this->input->get('rirj');
		$kd='LABS';
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtresep = $this->rmemdl->saveItemLab($kd,$nm);
			$dtlabheader_hariini = $this->rmemdl->panggillabheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtlabheader_hariini'] = $dtlabheader_hariini;
	...
	public function kirimkelab() {
		...
		if ($this->session->userdata("login") == TRUE) {
			...
			$dtresep = $this->rmemdl->proseskirimkelab($notransaksi_IN);
			$dtlabheader_hariini = $this->rmemdl->panggillabheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtlabheader_hariini'] = $dtlabheader_hariini;
	...
	public function kirimkerad() {
		...
		if ($this->session->userdata("login") == TRUE) {
			...
			$dtresep = $this->rmemdl->proseskirimkerad($notransaksi_IN);
			$dtradheader_hariini = $this->rmemdl->panggilradheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtradheader_hariini'] = $dtradheader_hariini;
	...
	public function editOrderLabHeader() { //order header lab
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
		$rirj = $this->input->get('rirj');
		if ($this->session->userdata("login") == TRUE) {
            ...
            $data['dtDPJP'] = $this->dokter->datadokter();
            $data['notransaksi_IN'] = $this->input->get('notransaksi_IN');
			$this->load->view('layanan/pelayanan/rme/instalasi/formEditOrderLab', $data);
		} else {
			redirect('login');
		}
	}

	public function saveEditHeaderLab() {
		$no_rm = $this->input->get('no_rm');
		$kode_dokter = $this->input->get('kode_dokter');
		$notransaksi = $this->input->get('notransaksi');
		$notransaksi_IN = $this->input->get('notransaksi_IN');
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtresep = $this->rmemdl->saveEditLabHeader($notransaksi_IN);
			$dtlabheader_hariini = $this->rmemdl->panggillabheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtlabheader_hariini'] = $dtlabheader_hariini;
	...
	public function saveEditHeaderRad() {
		$no_rm = $this->input->get('no_rm');
		$kode_dokter = $this->input->get('kode_dokter');
		$notransaksi = $this->input->get('notransaksi');
		$notransaksi_IN = $this->input->get('notransaksi_IN');
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtresep = $this->rmemdl->saveEditRadHeader($notransaksi_IN);
			$dtradheader_hariini = $this->rmemdl->panggilradheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtradheader_hariini'] = $dtradheader_hariini;
	...
	public function panggilFormCatatObatoral() {
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmeigdmdl");
			$dtcatatobat = $this->rmeigdmdl->CatatObatOral();
			$dtcatatobatdetail = $this->rmeigdmdl->CatatObatOraldetail();
			$jumlahbarisTanggal = $this->rmeigdmdl->cariJumlahBarisTanggal();

			$data["dtcatatobat"] = $dtcatatobat;
			$data["dtcatatobatdetail"] = $dtcatatobatdetail;
			$data["jumlahbarisTanggal"] = $jumlahbarisTanggal;
			
			// $data["satu"] = 1;
			$this->load->view('layanan/pelayanan/rmeigd/formCatatObatOral', $data);
		} else {
			redirect('login');
		}
	}

	public function loaddataobatoral() {
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmeigdmdl");
			$dtcatatobat = $this->rmeigdmdl->loaddatareseporal();
			$data["dtcatatobat"] = $dtcatatobat;
			$dt["dtview"] = $this->load->view('layanan/pelayanan/rmeigd/tdlistreseprekon_pulang', $data, TRUE);
			echo json_encode($dt);
		} else {
	...
	public function refreshTabelHisLab() {
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
		$notransaksi_IN = $this->input->get('notransaksi_IN');
		$kd='LABS';
		...
		if ($this->session->userdata("login") == TRUE) {
			...
			$dt["dtview"] = $this->load->view('layanan/pelayanan/rmeigd/tdlistlabHis', $data, TRUE);
			echo json_encode($dt);
		} else {
			redirect('login');
		}
	...
	public function tambahOrderRadHeader() { //order header lab
		if ($this->session->userdata("login") == TRUE) {
            $this->load->model("dokter");
            $data['dtDPJP'] = $this->dokter->datadokter();
            $data['kode_dokter_cek'] = $this->input->get('kode_dokter');
            ...
            $data['rirj'] = $this->input->get('rirj');
            $data['tgl_masuk'] = $this->input->get('tgl_masuk');
			$this->load->view('layanan/pelayanan/rme/instalasi/formOrderRad', $data);
		} else {
			redirect('login');
		}
	}

	public function saveOrderHeaderRad() {
		$no_rm = $this->input->get('no_rm');
		$kode_dokter = $this->input->get('kode_dokter');
		$notransaksi = $this->input->get('notransaksi');
		$rirj = $this->input->get('rirj');
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtresep = $this->rmemdl->saveRadHeader($kd,$nm);
			$dtradheader_hariini = $this->rmemdl->panggilradheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtradheader_hariini'] = $dtradheader_hariini;
	...
	public function hapusRadHeader() {
		$no_rm = $this->input->get('no_rm');
		$kode_dokter = $this->input->get('kode_dokter');
		$notransaksi = $this->input->get('notransaksi');
		$notransaksi_IN = $this->input->get('notransaksi_IN');
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtresep = $this->rmemdl->hapusRadHeader($notransaksi_IN);
			$dtradheader_hariini = $this->rmemdl->panggilradheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtradheader_hariini'] = $dtradheader_hariini;
	...
	public function tambahOrderRad() { //obat baru yg sebelumnya tdk ada di histori resep pasien
		if ($this->session->userdata("login") == TRUE) {
            $this->load->model("rmemdl");
            $data['dtpemeriksaanlab'] = $this->rmemdl->ambildatatindakanrad_rme();
			$data['no_rm'] = $this->input->get('no_rm');
			$data['notransaksi'] = $this->input->get('notransaksi');
			$data['notransaksi_IN'] = $this->input->get('notransaksi_IN');
			$this->load->view('layanan/pelayanan/rme/formTambahPemeriksaanRad', $data);
		} else {
			redirect('login');
		}
	}
	
	public function saveItemRad() {
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
		$rirj = $this->input->get('rirj');
		$kd='RDGL';
		...
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("rmemdl");
			$dtresep = $this->rmemdl->saveItemRad($kd,$nm);
			$dtradheader_hariini = $this->rmemdl->panggilradheader_hariini_ranap($no_rm,$notransaksi);
			$data['dtradheader_hariini'] = $dtradheader_hariini;
	...
	public function editOrderRadHeader() { //order header lab
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
		$rirj = $this->input->get('rirj');
		if ($this->session->userdata("login") == TRUE) {
            ...
            $data['dtDPJP'] = $this->dokter->datadokter();
            $data['notransaksi_IN'] = $this->input->get('notransaksi_IN');
			$this->load->view('layanan/pelayanan/rme/instalasi/formEditOrderRad', $data);
		} else {
			redirect('login');
		}
	}
	...
	public function hasilLabKlinik_rme($notransaksi_IN) {
		if ($this->session->userdata("login") == TRUE) {
			$r = date("Ymd");
			$data["notransaksi_IN"] = $notransaksi_IN;
			$this->load->library('pdf');
			$this->pdf->setPaper('F4', 'potrait');
			$this->pdf->filename = "cetakhasillab".$r.".pdf";
			$this->pdf->load_view('laporan/sampul/formathasillabklinik', $data);
			$this->pdf->render();
			$this->pdf->output();
		} else {
			redirect('login');
	...
	public function hasilLabKlinik_rme_anatomi($notransaksi_IN) {
		if ($this->session->userdata("login") == TRUE) {
			$r = date("Ymd");
			$data["notransaksi_IN"] = $notransaksi_IN;
			$this->load->library('pdf');
			$this->pdf->setPaper('F4', 'potrait');
			$this->pdf->filename = "cetakhasillab".$r.".pdf";
			$this->pdf->load_view('laporan/sampul/hasilanatomi', $data);
			$this->pdf->render();
			$this->pdf->output();
		} else {
			redirect('login');
	...
	public function refreshTabelHisRad() {
		$no_rm = $this->input->get('no_rm');
		$notransaksi = $this->input->get('notransaksi');
		$notransaksi_IN = $this->input->get('notransaksi_IN');
		$kd='LABS';
		...
		if ($this->session->userdata("login") == TRUE) {
			...
			$dt["dtview"] = $this->load->view('layanan/pelayanan/rmeigd/tdlistradHis', $data, TRUE);
			echo json_encode($dt);
		} else {
			redirect('login');
		}
