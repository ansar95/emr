...
//            $dt["hasil"] = $datarekap;
//            $data["total"] = $datatotal->total;
//            $data["dtview"] = $this->load->view('layanan/pelayanan/billing/pelayanan/tddaftarbilling', $dt, TRUE);
    ...
    public function download() {
        ...
        try {
            ...
            $filePath =APPPATH.'../assets/pdf';
            $fileToZip = [];

            $data= $this->billingmdl->getTransaksiEKG(2022);
            // print_r($data);die;
            foreach ($data as $value) {
                $fileToZip[] = $this->createPdf($value['notransaksi'], $filePath);
            }

            // $zip_name = $this->createZip($fileToZip,$filePath);
    ...
    public function createZip($fileToZip,$filePath)
    {
        foreach ($fileToZip as $file_name) {
            // $this->generate_pdf($file_name); // Generate the PDF
            
            // $pdf_file_path = APPPATH . 'pdf/' . $file_name . '.pdf';
            $this->zip->add_data($file_name, file_get_contents($filePath.'/'.$file_name)); // Add the PDF to the zip archive
        }
        
        // Save the zip file
        $zip_name = 'pdf_files.zip';
        $this->zip->archive($filePath.'/'. $zip_name);
        return $zip_name;
    }

    public function createPdf($nomor_transaksi,$filePath)
    {
        $logoPath = base_url('/assets/img/Untitled-1.png');
        $typeFile = pathinfo($logoPath, PATHINFO_EXTENSION);
        $logoFile = file_get_contents($logoPath);
        $logo = 'data:image/' . $typeFile . ';base64,' . base64_encode($logoFile);

        $fileName = "billing_".$nomor_transaksi.".pdf";
        $this->pdf->setPaper('A4', 'potrait');
        // $this->pdf->filename = "billing.pdf";
        // $content = $this->load->view('laporan/laporanbilling/example', [
        //     'nomor_transaksi'=>$nomor_transaksi,
        //     'logo'=>$logo
        // ], true);
        $content = $this->load->view('laporan/laporanbilling/cetak_billing_to_zip', [
            'nomor_transaksi'=>$nomor_transaksi,
            'logo'=>$logo
        ], true);
