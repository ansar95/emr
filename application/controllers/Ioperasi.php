...
class Ioperasi extends CI_Controller {
	...
	public function caridatarmjadwalopr() {
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("iopr");
			$this->load->model("unit");
			$dt["dtpasien"] = $this->iopr->pencarianrmjadwalopr();
			$dt["dtunit"] = $this->unit->unitjadwaloperasi();
	...
	public function jadwaloperasi() {
		if ($this->session->userdata("login") == TRUE) {
			...
			$unitinstalasi = $this->iopr->kamaropr();
			$data["unitinstalasi"] = $unitinstalasi;
			$data['include'] = "layanan/pelayanan/instalasi/operasi/jadwaloperasi";
			$data['menusamping'] = "menuopr";
			$data['backhome'] = "inst";
			$data['js'] = "pelayanan/instalasi/listopr_jadwal";
			$data['css'] = "pelayanan/instalasi/listopr";
			$this->load->view('templatesidebar/content', $data);
		} else {
			redirect('login');
	...
	public function caridatajadwalopr() {
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("iopr");
			$dtpasien = $this->iopr->datajadwalopr();
			$data["hasil"] = $dtpasien;
			$this->load->view('layanan/pelayanan/instalasi/operasi/tddtlistjadwalopr', $data);
		} else {
			redirect('login');
		}
	}
	...
	public function panggiltambahjadwalopr() {
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("dokter");
			$this->load->model("iopr");
			$this->load->model("unit");
			...
			$dtdokterpemeriksa = $this->dokter->datadokterpemeriksa();
			$unitpelaksana = $this->iopr->fullopr();
			$unitpemesan = $this->unit->unitjadwaloperasi();
			$tindakan = $this->tindakan->ambildatatindakanopr();
			$golongan = $this->golongan->listgolongan();
			$data["dtdokterpengirim"] = $dtdokterpengirim;
			$data["dtdokterpemeriksa"] = $dtdokterpemeriksa;
			$data["dtunitpelaksana"] = $unitpelaksana;
			$data["dtunitpemesan"] = $unitpemesan;
			$data["dtgolongan"] = $golongan;
			$data["dttindakan"] = $tindakan;
			$data["formpilih"] = 0;
			$this->load->view('layanan/pelayanan/instalasi/operasi/formtambahjadwalopr', $data);
		} else {
			redirect('login');
		}
	}

	public function simpanjadwaloperasi() {
		if ($this->session->userdata("login") == TRUE) {
			$this->load->model("iopr");
			$dtnotrans = $this->iopr->ambilkodebookingoperasi();
			$dtsimpan = $this->iopr->simpanjadwaloprbaru($dtnotrans);
			$dtjadwal = $this->iopr->datajadwalopr();
			$dt["stat"] = $dtsimpan;
			$dt["dtnotrans"] = $dtnotrans[2];
			$dt["dtjadwal"] = $dtjadwal;
			echo json_encode($dt);
		} else {
			redirect('login');
		}
	...
	public function hapusjadwaloperasi() {
        if ($this->session->userdata("login") == TRUE) {
            $this->load->model("iopr");
            $dtsimpan = $this->iopr->hapusjadwalopr();
            $dt["stat"] = $dtsimpan;
            // $dt["dtview"] = $this->load->view('layanan/pelayanan/instalasi/operasi/tddtlistopr', $data, TRUE);
            // if ($dtsimpan == true) {
            //     $dt["info"] = "Data telah Terhapus";
            // } else {
            //     $dt["info"] = "Tidak dapat menghapus";
            // }
            echo json_encode($dt);
        } else {
            redirect('login');
	...
	public function panggileditjadwalopr() {
        if ($this->session->userdata("login") == TRUE) {
			$this->load->model("dokter");
			$this->load->model("iopr");
			$this->load->model("unit");
			...
			$data["dttindakan"] = $tindakan;
			$data["formpilih"] = 1;
            $data["dt"] = $this->iopr->ambileditjadwalopr();
			$this->load->view('layanan/pelayanan/instalasi/operasi/formtambahjadwalopr', $data);
        } else {
            redirect('login');
        }
	...
	public function ubahjadwalopr() {
        if ($this->session->userdata("login") == TRUE) {
            $this->load->model("iopr");
            $dtsimpan = $this->iopr->ubahjadwalopr();
            $dt["stat"] = $dtsimpan;
            echo json_encode($dt);
        } else {
            redirect('login');
