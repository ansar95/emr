	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$("#caridata").click(function(e) {
				...
				$.ajax({
					url: "<?php echo site_url(); ?>/registrasipasien/caripasienregisuri",
					type: "GET",
					data: {
						np: np,
						pp: pp,
		...
		function panggilinputuri(id) {
			$.ajax({
				...
				success: function(result) {
					...
					if (result == 0) {
						$.ajax({
							url: "<?php echo site_url(); ?>/registrasipasien/inputregisuri",
							type: "GET",
							data: {
								id: id
							},
		...
		function panggilinputuri_old(id) {
			$.ajax({
				url: "<?php echo site_url(); ?>/registrasipasien/inputregisuri",
				type: "GET",
				data: {
			...
			$.confirm({
				...
				buttons: {
					somethingElse: {
						...
						keys: ['enter', 'shift'],
						action: function() {
							window.location.href = "<?php echo site_url(); ?>/registrasipasien/dataregistrasiuri";
						}
					}
				}
			});
