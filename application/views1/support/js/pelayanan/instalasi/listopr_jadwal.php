...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$("#tambahlab").click(function(e) {
			$.ajax({
				url: "<?php echo site_url();?>/ioperasi/panggiltambahjadwalopr",
				type: "GET",
				success: function (ajaxData){
					hapusload();
	...
	function panggiledit(id) {
        ...
        var kdunit = 'IBSS';
        $.ajax({
            url: "<?php echo site_url();?>/ioperasi/panggileditjadwalopr",
            type: "GET",
            data : {id: id, kdunit: kdunit},
            success: function (ajaxData){
                hapusload();
    ...
    function hapusinst(e, id) {
        var rm = $(e).parents('tr').find("td:eq(2)").text();
        var tgl = $(e).parents('tr').find("td:eq(1)").text();
        // var idnya = $(e).parents('tr').find("td:eq(0)").text();
        var kdunit = 'IBSS';
        $.confirm({
            title: 'Hapus Data',
            content: 'Yakin Pasien dengan no RM ' + rm + ' Kode Booking ' + tgl + ' Dihapus?',
            buttons: {
                batal: {
                    text: 'Batal',
                    btnClass: 'btn-red',
                ...
                kembali: {
                    ...
                    action: function () {
                        ...
                        // var nrp = document.getElementById("nrp").value;
                        $.ajax({
                            url: "<?php echo site_url();?>/ioperasi/hapusjadwaloperasi",
                            type: "GET",
                            data: {
                                id: id
                            },
