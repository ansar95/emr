...
<table class="tabelbiasa2" width=100%  align="left" cellpadding="3" cellspacing="1">
  <tbody>
    <?php
        ...
        if ($qtysaja ==  "on" ) {
          $per="SELECT kartustok_gudang.kodeobat as kodeobat,mobat.namaobat as namaobat, mobat.satuanpakai as satuanpakai, kartustok_gudang.harga as harga  FROM kartustok_gudang,mobat WHERE kartustok_gudang.kodeobat=mobat.kodeobat and (kartustok_gudang.masuk<>0 or kartustok_gudang.keluar<>0 or kartustok_gudang.returm<>0 or kartustok_gudang.returk<>0) ".$filter." group by kartustok_gudang.kodeobat order by mobat.namaobat";
         } else {
          $per="SELECT kartustok_gudang.kodeobat as kodeobat,mobat.namaobat as namaobat, mobat.satuanpakai as satuanpakai, kartustok_gudang.harga as harga, kartustok_gudang.pendanaan as pendanaan FROM kartustok_gudang,mobat WHERE kartustok_gudang.kodeobat=mobat.kodeobat and (kartustok_gudang.masuk<>0 or kartustok_gudang.keluar<>0 or kartustok_gudang.returm<>0 or kartustok_gudang.returk<>0) ".$filter." group by pendanaan, kartustok_gudang.kodeobat, kartustok_gudang.harga order by mobat.namaobat";
         }
 ...
 ?>
       ...
       $i=0;
        foreach ($qry->result_array() as $brs) {
            ...
            $sawal=0;
            if ($qtysaja ==  "on" ) {
              $perintah1="SELECT  sum(masuk-keluar+returm-returk) as sawal FROM kartustok_gudang where kodeobat='$kodeobat' and tanggal<'$tanggalawal' group by kodeobat, harga";
            ...
            //datanya
            if ($qtysaja ==  "on" ) {
              $per2="SELECT sum(masuk) as jummasuk, sum(keluar) as jumkeluar, sum(returm) as jumreturm, sum(returk) as jumreturk FROM kartustok_gudang where kodeobat='$kodeobat' and tanggal>='$tanggalawal' and tanggal<='$tanggalakhir' ";
              $digrup=" group by kodeobat";
            } else {
            ...
            if ($masuk != 0 || $keluar != 0 || $sawal != 0) {
              $i++;

         ?>  
            <tr style="font-family: Arial; font-size: 11px; text-align: center;">
              <td width="3%" style="text-align: right" ><?php echo (int)$i.'.'; ?></td>
              <td style="text-align: left" ><?php echo $namaobat; ?></td>
              <td width="8%" style="text-align: left" ><?php echo $satuanpakai; ?></td>
              <td width="8%" style="text-align: right" ><?php echo $cekprinternya == 3 ? (int)$sawal : formatuang($sawal); ?></td>
              <td width="8%" style="text-align: right" ><?php echo $cekprinternya == 3 ? (int)$masuk : formatuang($masuk); ?></td>
              <td width="8%" style="text-align: right" ><?php echo $cekprinternya == 3 ? (int)$keluar : formatuang($keluar); ?></td>
              <td width="8%" style="text-align: right" ><?php echo $cekprinternya == 3 ? (int)$jumlahstok : formatuang($jumlahstok); ?></td>
              <?php if ($qtysaja != "on" ) { ?>
                <td width="10%" style="text-align: right" ><?php echo $cekprinternya == 3 ? (int)$harga : formatuang_dec($harga); ?></td>
                <td width="12%" style="text-align: right" ><?php echo $cekprinternya == 3 ? (int)$nilaistok : formatuang_dec($nilaistok); ?></td>
              <?php } ?>
            </tr>
          <?php
        } }
...
<?php if (($masuk != 0 || $keluar != 0 || $sawal != 0) and $qtysaja !=  "on") { ?>
<table width="100%" border="0" cellpadding="3" cellspacing="1">
  <tbody>
    <tr style="font-family: Gotham, 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 11px; text-align: center;">
      <th style="text-align: right" >TOTAL</th>
      <!-- <th width="8%" style="text-align: right" ><?php echo formatuang_dec($totaljumlah); ?></th> -->
      <th width="8%" style="text-align: right" ><?php echo $cekprinternya == 3 ? (int)$totaljumlah : formatuang_dec($totaljumlah); ?></th>
    </tr>
  </tbody>
</table>
