		...
		<div class="col-12">
			...
			<nav>
				<div class="nav nav-tabs font-weight-bold border-bottom ml-0" id="nav-tab" role="tablist">
					...
					<a class="nav-item nav-link mt-0 bg-warning text-dark" id="igd" data-toggle="tab" href="#navIGD" role="tab" aria-controls="navIGD" aria-selected="false" style="color: red; font-size: 12px;">IGD Gawat Darurat</a>
					...
					<a class="nav-item nav-link mt-0 text-white" id="rajalOld" data-toggle="tab" href="#navRajalOld" role="tab" aria-controls="navRajalOld" aria-selected="false" style="color: red; font-size: 12px; background-color: #7A8407;">History Pasien Rajal</a>
				</div>
			</nav>
				...
				<div class="tab-content mb-5" id="nav-tabContent">
					...
					<div class="tab-pane fade" id="navRawatInap" role="tabpanel" aria-labelledby="rawatInap">
						...
						<div class="row col-md-12 mt-3">
							...
							?>
								<div class="col-md-9">
									<div class="card border-danger custom-cardranap" onmouseover="changeBackgroundColor(this, true)" onmouseout="changeBackgroundColor(this, false)" <?php if ($row->status == 1) echo 'style="background-color: #FDF5E6;"'; ?>>
										<div class="card-body">
											<input class="form-control id-input" type="hidden" value="<?php echo $row->id; ?>">
											<input class="form-control notransaksi-input" type="hidden" value="<?php echo $row->notransaksi; ?>">
											<input class="form-control nama-input" type="hidden" value="<?php echo $row->nama_pasien; ?>">
											...
											<button class="btn btn-primary float-right" onclick="bukaFormPasienRanap('<?php echo $row->id; ?>', '<?php echo $row->notransaksi; ?>', '<?php echo $row->nama_pasien; ?>', '<?php echo $row->no_rm; ?>', '<?php echo $row->kode_dokter; ?>',3,1)">Buka</button>
											<h6 class="card-title"><?php echo $row->no_rm.' - '.$row->nama_pasien; ?></h6>
...
function bukaFormPasienRanap(id, notransaksi, nama_pasien, no_rm, kode_dokter,angkatab,editing) {
...
form.submit();
	...
	function filterDataRanap() {
        var input, filter, cards, card, cardTitle, cardNoRM, cardNama;
        input = document.getElementById("cariInputRanap");
        filter = input.value.toLowerCase();
        cards = document.getElementsByClassName("custom-cardRanap");
