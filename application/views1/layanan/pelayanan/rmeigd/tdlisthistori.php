		...
		<div class="card border-secondary">
			<div class="card-body">
				...
				<?php if ($kelompok_unit  =='POLI') { ?>
					<button class="btn btn-primary float-right" onclick="bukaFormPasien('<?php echo $row->id; ?>', '<?php echo $row->notransaksi; ?>', '<?php echo $row->nama_pasien; ?>', '<?php echo $row->no_rm; ?>', '<?php echo $row->kode_dokter; ?>',1,0)"><?php echo 'Poli'; ?></button>
				<?php } ?>

				<?php if ($kelompok_unit  == 'RUANGAN') { ?>
					<!-- bukaFormPasienRanap(id, notransaksi, nama_pasien, no_rm, kode_dokter,status,angkatab,editing) -->
					<button class="btn btn-primary float-right" onclick="bukaFormPasienRanap('<?php echo $row->id; ?>', '<?php echo $row->notransaksi; ?>', '<?php echo $row->nama_pasien; ?>', '<?php echo $row->no_rm; ?>', '<?php echo $row->kode_dokter; ?>',1,0)"><?php echo 'Perawatan'; ?></button>
				<?php } ?>

				<?php if ($kelompok_unit  == 'IGD') { ?>
					<button class="btn btn-primary float-right" onclick="bukaFormPasienIgd('<?php echo $row->id; ?>', '<?php echo $row->notransaksi; ?>', '<?php echo $row->nama_pasien; ?>', '<?php echo $row->no_rm; ?>', '<?php echo $row->kode_dokter; ?>',1,0)"><?php echo 'IGD'; ?></button>
				<?php } ?>
