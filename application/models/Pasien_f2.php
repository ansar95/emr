...
class Pasien extends CI_Model {
	...
	public function simpanregisuri($dtnotrans, $hasilicd) {
		...
		$simpanregisdetail = array(
			'no_rm' => $this->input->get("rm"), 
			'kode_kelas' => $this->input->get("kp"),
			'nama_kelas' => $this->input->get("kptext"),
			'kode_unit' => $this->input->get("tp"), 
			'nama_unit' => $this->input->get("tptext"), 
			'tgl_masuk_kamar' => $tglm, 
			'tgl_keluar_kamar' => "0000-00-00", 
...
// =================DISINI OTOMATIS RADIOLOGI ========
	...
	function fetch_pasien_pengelolaan($limit, $start) {
		...
		<tr>
			...
			<th width="14%">Unit Perawatan</th>
			<th width="11%">Kamar</th>
			<th width="7%"><div align="center">Msk Kamar</div></th>
			<th width="4%"><div align="center">Jam</div></th>
			<th width="26%"><div align="center">Proses</div></th>
		...
		<tr>
			...
			<td><div align="center">'.$row->jam_masuk.'</div></td>
			<td class="text-center">
				<button onclick="panggilpindahkamar(\''.$row->notransaksi.'\');" class="btn btn-sm btn-primary">Pindah</button>
				<button onclick="cetakregisranap(\''.$row->id.'\')" class="btn btn-sm btn-info">Regis</button>
				<button onclick="pulangkan(\''.$row->notransaksi.'\');" class="btn btn-sm btn-success">Pulang</button>
				<button onclick="pasienkembali(this, \''.$row->id.'\');" class="btn btn-sm btn-danger">Kembali</button>
				<button onclick="panggillihat(\''.$row->no_rm.'\')" class="btn btn-sm btn-warning">History</button>
			</td>
			<td class="text-center">
				<button onclick="cetakgelangbayi(\''.$row->idreg.'\')" class="btn btn-sm btn-success">GL</button>
			</td>		
		</tr>
		';
		}
    ...
    public function kodekirimberkas() {
        ...
        $idnya = $this->input->get("id");
		var_dump($idnya);
		$this->db->query("UPDATE register set berkas=1 where id='$idnya' LIMIT 1");
	}
	...
	public function kodetidakadaberkas() {
        ...
        $idnya = $this->input->get("id");
		var_dump($idnya);
		$this->db->query("UPDATE register set berkas=3 where id='$idnya' LIMIT 1");
	}
	...
	public function kodekerawatinap() {
        ...
        $idnya = $this->input->get("id");
		var_dump($idnya);
		$this->db->query("UPDATE register set berkas=4 where id='$idnya' LIMIT 1");
	}
	...
	public function kodekembaliberkas() {
        ...
        $idnya = $this->input->get("id");
		var_dump($idnya);
		$this->db->query("UPDATE register set berkas=6 where id='$idnya' LIMIT 1");
	}
}
