...
class Apotik extends CI_Model {
    ...
    public function resepselesai_1() {
        $id = $this->input->get("id");

        $this->db->select("idnoresep, noresep, notransaksi");
		$this->db->from("resep_header");
		$this->db->where('idnoresep', $id);
        $databasic = $this->db->get();
        $datasinggle = $databasic->row();
    ...
    public function ambildetail($id) {
        $this->db->select("namaobat, satuanpakai, qty, hargapakai, tuslag, jumlah, dosis, noninacbg, idnoresep, proses, pagi, siang, malam, takaran, caramakan, keterangan, pendanaan, catatanresep,dosis ");
        $this->db->from("resep_detail");
    ...
    public function simpanresep($dtnotrans) {
        ...
        if ($noresep == "") {
            ...
            //isi task id 6 disini
            //-->cari dulu kodebooking dari notransaksi
            $kodebooking='';
            $qry21=$this->db->query("SELECT kode_booking FROM register where  notransaksi= '$notrans' LIMIT 1");
            foreach ($qry21->result_array() as $brs21) {
                $kodebooking=$brs21['kode_booking'];
            }      
            $tgljamsekarang=date('Y-m-d  H:i:s');
            $qry21=$this->db->query("INSERT INTO y_updatewaktu (id,kodebooking,taskid,waktu,flag) VALUES (NULL, '$kodebooking', '6', '$tgljamsekarang', '0')");
    ...
    public function ambilresep() {
        ...
        $date = date_create($this->input->get("tgl"));
        $tgl = date_format($date,"Y-m-d");
        $this->db->select("resep_header.*, pasien.nama_pasien as nama, pasien.alamat as alamat, pasien.telp as telp, urutdepo");
        $this->db->from("resep_header");
        $this->db->join("pasien", "resep_header.no_rm = pasien.no_rm");
        $this->db->where("shift", $shift);
        $this->db->where("kode_depo", trim($depo));
        $this->db->where("tglresep", $tgl);
		$this->db->order_by("urutdepo desc");
    ...
    public function ambildataresepheader($id) {
        $this->db->select("resep_header.*, resep_header.nama_umum as nama_umum, pasien.nama_pasien as nama");
        $this->db->from("resep_header");
        $this->db->join("pasien", "resep_header.no_rm = pasien.no_rm");
        $this->db->where("idnoresep", $id);
        $data = $this->db->get();
    ...
    public function isidepo_panggil_per_unit() {
        $shift = $this->input->get("shift");
        $kodedp = $this->input->get("depo");
        $kode_depo =trim($kodedp);
        $kode_unit = $this->input->get("unit");
        ...
        if($cek->num_rows()>0) {
            ...
            $user2= $this->session->userdata("nmuser");
            $dt = $this->db->query("update resep_header set kode_depo='$kode_depo', shift='$shift', nama_depo='$nama_unit' , user2='$user2', dariunit=1, urutdepo='$dtskrg' where no_rm='$no_rm' and kode_unit='$kode_unit' and kode_depo='' ");
            return $dt;
        } else {
            return false;
        }
    ...
    public function isidepo() {
        ...
        $dtskrg=date("Y-m-d H:i:s");
        //--cek dulu apakah sdh ada yg add sebelumnya
        // $cek = $this->db->query("select no_rm from resep_header where no_rm='$no_rm' and kode_unit='$kode_unit' and kode_depo='' ");
        $cek = $this->db->query("select no_rm from resep_header where no_rm='$no_rm' and kode_depo='' ");
        if($cek->num_rows()>0) {
            $cek = $this->db->query("select nama_unit from munit where kode_unit='$kode_depo' limit 1 ");
            foreach ($cek->result_array() as $brs2) {
                $nama_unit=$brs2['nama_unit'];
            ...
            $user2= $this->session->userdata("nmuser");
            // $dt = $this->db->query("update resep_header set kode_depo='$kode_depo', shift='$shift', nama_depo='$nama_unit' , user2='$user2', dariunit=1, urutdepo='$dtskrg' where no_rm='$no_rm' and kode_unit='$kode_unit' and kode_depo='' ");
            $dt = $this->db->query("update resep_header set kode_depo='$kode_depo', shift='$shift', nama_depo='$nama_unit' , user2='$user2', dariunit=1, urutdepo='$dtskrg' where no_rm='$no_rm' and  kode_depo='' ");

            $cek = $this->db->query("select noresep from resep_header  where no_rm='$no_rm' and  kode_depo='$kode_depo' ");
            foreach ($cek->result_array() as $brs2) {
                $noresep=$brs2['noresep'];
                $dt2 = $this->db->query("update resep_detail set proses=1 where noresep='$noresep' and proses=0 ");    
            }

            $cek = $this->db->query("select notransaksi from resep_header  where no_rm='$no_rm' and  kode_depo='$kode_depo' and tglresep='$tgl' ");
            foreach ($cek->result_array() as $brs2) {
                $notrans=$brs2['notraksaksi']; 
            }
            //taskid 6
            ...
            $tgljamsekarang=date('Y-m-d  H:i:s');
            $qry21=$this->db->query("INSERT INTO y_updatewaktu (\kodebooking,taskid,waktu,flag) VALUES ('$kodebooking', '6', '$tgljamsekarang', '0')");
