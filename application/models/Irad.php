...
class Irad extends CI_Model {

	public function datarad() {
	...
	public function simpanradbaru($dtnotrans) {
		...
		$datasimpan = array(
			...
			'kode_analis' => "",
			'nama_analis' => "",
			'kode_radiografer' => $this->input->get("radiografer"),
			'nama_radiografer' => $this->input->get("radiografertext"),
			'kode_hemodialis' => "",
			'nama_hemodialis' => "",
			'kode_kelas' => "", 
			'nama_kelas' => "", 
	...
	public function simpantindakanrad($dtpasien, $dthargatindakan, $ambilnotrans) {
		...
		$datasimpan = array(
			...
			'cjk' => "", 
			'calamat' => "", 
			'cjenispemeriksaan' => $this->input->get("jpemeriksaan"), 
			'ctanggal' => "", 
			'cnofoto' => $this->input->get("nofoto"), 
			'cruangan' => "", 
			'cklinis' => $this->input->get("klinis"), 
			'cdokter' => "", 
			'judul' => "", 
			'Hepar' => "", 
			'GB' => "", 
			...
			'ren' => "", 
			'Urinaria' => "", 
			'hasil_pemeriksaan' => $this->input->get("hasilperiksa"), 
			'kesan' => $this->input->get("kesan"),
			'jenis' => "0", 
			'tambahan' => "", 
			'jam' => date("H:i:s"), 
			'qty' => "1",
	...
	public function ambileditdatatindakan() {
		$id = $this->input->get("id");
		$this->db->select("mtindakan.tindakan as tindakan, mtindakan.kode_tindakan as kode_tindakan, mtindakan.jasas as jasas, ptindakan_instalasi.id as id,ptindakan_instalasi.nonasuransi as nonasuransi, ptindakan_instalasi.diskon as diskon, ptindakan_instalasi.hasil_pemeriksaan as hasil_pemeriksaan, ptindakan_instalasi.kesan as kesan, ptindakan_instalasi.cjenispemeriksaan as jpemeriksaan, ptindakan_instalasi.cnofoto as nofoto, ptindakan_instalasi.cklinis as klinis ");
		$this->db->from("ptindakan_instalasi");
		$this->db->join("mtindakan", "mtindakan.kode_tindakan = ptindakan_instalasi.tindakan");
	...
	public function simpanbhprad($dtpasien) {
    ...
    public function hapusinstalasi() {
        $id = $this->input->get("id");
        $this->db->select("register_instalasi.notransaksi_IN as notransaksi_IN, register_instalasi.notransaksi as notransaksi,register_instalasi.kode_unit as kode_unit, munit.rad as rad");
        $this->db->from("register_instalasi");
