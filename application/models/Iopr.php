...
class Iopr extends CI_Model {
	...
	public function datajadwalopr() {
		$this->db->select("jadwaloperasi.kodebooking as kodebooking, jadwaloperasi.tanggaloperasi as tanggaloperasi, jadwaloperasi.jenistindakan as tindakan, jadwaloperasi.namapoli as namapoli, jadwaloperasi.kodepoli as kodepoli, jadwaloperasi.terlaksana as terlaksana, jadwaloperasi.no_rm as no_rm, pasien.nama_pasien as nama_pasien, pasien.no_askes as no_askes, jadwaloperasi.nama_dokter as nama_dokter, jadwaloperasi.id as id");
		$this->db->from("jadwaloperasi");
		$this->db->join("pasien", "pasien.no_rm = jadwaloperasi.no_rm");
		$this->db->where("jadwaloperasi.terlaksana", 0);
		$data = $this->db->get();
		return $data->result();
	}

	public function datajadwalopr_display() {
		$tgl = date("Y-m-d");
		$this->db->select("jadwaloperasi.kodebooking as kodebooking, jadwaloperasi.tanggaloperasi as tanggaloperasi, jadwaloperasi.jenistindakan as tindakan, jadwaloperasi.namapoli as namapoli, jadwaloperasi.kodepoli as kodepoli, jadwaloperasi.terlaksana as terlaksana, jadwaloperasi.no_rm as no_rm, pasien.nama_pasien as nama_pasien, pasien.no_askes as no_askes, jadwaloperasi.nama_dokter as nama_dokter, jadwaloperasi.id as id");
		$this->db->from("jadwaloperasi");
		$this->db->join("pasien", "pasien.no_rm = jadwaloperasi.no_rm");
		// $this->db->where("jadwaloperasi.terlaksana", 0);
		$this->db->where("jadwaloperasi.tanggaloperasi", $tgl);
		$data = $this->db->get();
		return $data->result();
	}
	...
	public function pencarianrmjadwalopr() {
		$no = $this->input->get("rm");
		$this->db->select("pasien.no_rm as no_rm, pasien.nama_pasien as nama_pasien, pasien.no_askes as no_askes");
		$this->db->from("pasien");
		$this->db->where("pasien.no_rm", $no);
		$data = $this->db->get();
		return $data->result();
	}
	...
	public function ambilkodebookingoperasi() {
		$tgl = date("Y-m-d");
		$this->db->from("mtransaksiopr");
		$this->db->where("tgltransaksi", $tgl);
		$this->db->limit(1);
		$data = $this->db->get();
		if ($data->result() == NULL) {
			...
			$numdate = str_replace("-", "", $tgl);
			$numend = sprintf("%05d", $numstart);
			$numgabung = "FO" . substr($numdate, 2) . $numend;
			return array($tgl, $numstart, $numgabung);
		} else {
			$t = $data->row();
			$numstart = $t->nomor + 1;
	...
	public function simpantindakanopr($dtpasien, $dthargatindakan) {
		...
		$datasimpan = array(
			...
			'kode_perawat2' => $this->input->get("tdperawat2"),
            'nama_perawat2' => $this->input->get("tdperawattext2"),
		...
		$datasimpan2 = array(
            'jammulai' => $this->input->get("jammulai"),
            'jamselesai' => $this->input->get("jamselesai")
        );
        $this->db->where("notransaksi_IN", $this->input->get("tgl"));
        $dt2 = $this->db->update('register_instalasi', $datasimpan2);

		//cek jadwal operasi
		if ( $this->input->get("jammulai") != NUll and $this->input->get("jamselesai") != NULL) {
			$datasimpan3 = array(
				'terlaksana' => '1'
			);
			$this->db->where("no_rm", $this->input->get("rm") );
			$this->db->where("tanggaloperasi", $tgl );
			$this->db->where("terlaksana", 0 );
			$dt2 = $this->db->update('jadwaloperasi', $datasimpan3);
		}
		return $dt;
	}
	...
	public function simpanjadwaloprbaru($dtnotrans) {
		// $dt = $this->input->get("dataadd");
		$date = date_create($this->input->get("instgl"));
		$tgl = date_format($date,"Y-m-d");
		...
		$datasimpan = array(
			'kodebooking' => $dtnotrans[2],  
			'tanggaloperasi' => $tgl, 
			'jenistindakan' => $this->input->get("tindakantext"),
			'no_rm' => $this->input->get("rm"),
			'kodepoli' => $this->input->get("unitasal"), 
			'namapoli' => $this->input->get("unitasaltext"),
			'terlaksana' => 0, //date("Y-m-d"), 
			'kode_dokter' => $this->input->get("dpengirim"), 
			'nama_dokter' => $this->input->get("dpengirimtext"), 
			'rujukan' => $r,
			'golongan' => $this->input->get("gol") , 
		...
		if ($dtnotrans[1] == 1) {
			...
			$this->db->insert('mtransaksiopr', $dataubah);
		} else {
			$dataubah = array(
				"tgltransaksi" => $dtnotrans[0],
				"nomor" => $dtnotrans[1]
			...
			$this->db->where("tgltransaksi", $dtnotrans[0]);
			$this->db->update('mtransaksiopr', $dataubah);
		}

		$dt = $this->db->insert('jadwaloperasi', $datasimpan);
		return $dt;
	}

    public function hapusjadwalopr() {
            $id = $this->input->get("id");
            $this->db->where('id', $id);
            $this->db->delete('jadwaloperasi');
            return true;

    }

	public function ambileditjadwalopr() {
        $no = $this->input->get("id");

			$this->db->select("jadwaloperasi.kodebooking as kodebooking, jadwaloperasi.tanggaloperasi as tanggaloperasi, jadwaloperasi.jenistindakan as tindakan, jadwaloperasi.namapoli as namapoli, jadwaloperasi.kodepoli as kodepoli, jadwaloperasi.terlaksana as terlaksana, jadwaloperasi.no_rm as no_rm, pasien.nama_pasien as nama_pasien, pasien.no_askes as no_askes, jadwaloperasi.nama_dokter as nama_dokter, jadwaloperasi.kode_dokter as kode_dokter, jadwaloperasi.kode_unit_pelaksana as kode_unit_pelaksana, jadwaloperasi.id as id, jadwaloperasi.rujukan as rujukan, jadwaloperasi.golongan as golongan");
			$this->db->from("jadwaloperasi");
			$this->db->join("pasien", "pasien.no_rm = jadwaloperasi.no_rm");
			$this->db->where("jadwaloperasi.terlaksana", 0);
			$this->db->where("jadwaloperasi.id", $no);
			$data = $this->db->get();
        return $data->row();
	}

	public function ubahjadwalopr() {
		$date = date_create($this->input->get("tgli"));
		$tgl = date_format($date,"Y-m-d");
		if ($this->input->get("rujuk") == "UM") {
			$r = "";
        ...
        $this->db->where("id", $this->input->get("id"));
        $dt = $this->db->update('jadwaloperasi', $datasimpan);
        return $dt;
    }
