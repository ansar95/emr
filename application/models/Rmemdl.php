...
class Rmemdl extends CI_Model {
	...
	public function hapusdetailLab($id,$notransaksi_IN) {
        $this->db->where('id', $id);
        $dt = $this->db->delete('ptindakan_instalasi');
		$dataubah = array(
			"kirim" => 0,
			"jamkirim" => date("Y-m-d H:i:s"),
			"bellproses" => 0,
			"cekitemorder" => 1,	
		);
		$this->db->where("notransaksi_IN", $notransaksi_IN);
		$this->db->limit(1);
		$this->db->update('register_instalasi', $dataubah);
        return $dt;
    }

	public function hapusdetailRad($id,$notransaksi_IN) {
        $id = $this->input->get("id");
        $this->db->where('id', $id);
        $dt = $this->db->delete('ptindakan_instalasi');
		$dataubah = array(
	...
	public function simpanlaporanoperasi() {
		...
		$dataawal = array(
			...
			'jenis_anastesi' => $this->input->get("jenis_anastesi"),
	...
	public function panggillabheader_hariini_ranap($no_rm,$notransaksi) {
		$tglHariIni=date("Y-m-d");
		$this->db->from("register_instalasi");
		$this->db->where("no_rm", $no_rm);
		$this->db->where("notransaksi", $notransaksi);
		...
		if ($this->session->userdata("kodedokter") != 'XXXXXX'){
			$this->db->where("kode_dokter",$this->session->userdata("kodedokter") );
		}
		$this->db->order_by("notransaksi_IN", 'DESC');
		$data = $this->db->get();
		return $data->result();
	}

	public function panggillabheader_semua_ranap($no_rm,$notransaksi) {
		$this->db->from("register_instalasi");
		$this->db->where("no_rm", $no_rm);
		// $this->db->where("notransaksi", $notransaksi);
		$this->db->where("kode_unit", "LABS");
	...
	public function saveLabHeader($kd,$nm) {
		...
		$tglHariIni=date("Y-m-d");
			...
			$datasimpan = array(
				...
				'kode_unit' => $kd, 
				'nama_unit' => $nm,
				'kode_dokter' => $this->input->get("dokterf"), 
				'nama_dokter' => $this->input->get("dokterftext"), 
				'rujukan' => $this->input->get("rirj"), 
				'tgl_masuk' => $this->input->get("tgl_masuk"), 
				'kode_unitR' => $this->input->get("kode_unit"), 
				'nama_unitR' => $this->input->get("nama_unit"), 
				'user1' => $this->session->userdata("nmuser"), 
				...
				'idrinap' => "0", 
				'barulama' => "0",
				'pilihunitlab' => $this->input->get("pilihunitlab"),
				'cito' => $this->input->get("cito"),
				'diagnosa' => $this->input->get("diagnosa")
			);
			$result = $this->db->insert('register_instalasi', $datasimpan);
	...
	public function hapusLabHeader($id) {
		$this->db->select("id");
		$this->db->from("ptindakan_instalasi");
		$this->db->where("notransaksi_IN", $id);
		$data = $this->db->get();
		$baris=$data->num_rows();
		if ($baris == 0 ) {
			$this->db->where('notransaksi_IN', $id);
			$this->db->limit(1);
			$dt = $this->db->delete('register_instalasi');
		} else {	
			$dt=false;
		} 
		return $dt;
	...
	public function saveItemLab($kd,$nm) {
				$date = date_create($this->input->get("radtgl"));
				$tgl = date_format($date,"Y-m-d");
				$datasimpan = array(
					'kode_unit' => $kd, 
					...
					'no_rm' => $this->input->get('no_rm'),
					'tanggal' => $tgl, 
					'tindakan' => $this->input->get('kode_tindakan'),
					'jasas' => $this->input->get('jasas'),
					'jasap' => "0", 
					'jasam' => "0", 
					'jasaa' => "0",
					'perawatsaja' => "0", 
				...
				$this->db->where("notransaksi_IN", $this->input->get('notransaksi_IN'));
				$this->db->limit(1);
				$this->db->update('register_instalasi', $dataubah);
	}
	
	
	public function ambildatatindakanlab_rme() {
		$this->db->select("kode_tindakan, tindakan, jasas");
		$this->db->from("mtindakan");
		$this->db->where("kode_unit", "LABS");
		if ($this->input->get("pilihlab") == 1) {
			$this->db->where("klinik", 1);
		} elseif ($this->input->get("pilihlab") == 2) {
			$this->db->where("anatomi", 1);
		} else if ($this->input->get("pilihlab") == 3) {
			$this->db->where("mikro", 1);
		}
		$this->db->where("hapus", "0");
		$data = $this->db->get();
		return $data->result();
	...
	public function proseskirimkelab() {
	...
	public function proseskirimkerad() {
	...
	public function saveEditLabHeader($notransaksi_IN) {
			$dataubah = array(
				'tanggal' =>  $this->input->get("tanggalorder"), 
				'kode_dokter' => $this->input->get("dokterf"), 
				'nama_dokter' => $this->input->get("dokterftext"), 
				'user2' => $this->session->userdata("nmuser"), 
				'lastlogin' => date("Y-m-d H:i:s"), 
	...
	public function panggilradheader_hariini_ranap($no_rm,$notransaksi) {
		$tglHariIni=date("Y-m-d");
		$this->db->from("register_instalasi");
		$this->db->where("no_rm", $no_rm);
		$this->db->where("notransaksi", $notransaksi);
	...
	public function panggilradheader_semua_ranap($no_rm,$notransaksi) {
		$this->db->from("register_instalasi");
		$this->db->where("no_rm", $no_rm);
		// $this->db->where("notransaksi", $notransaksi);
		// $this->db->where("kode_unit", "RDGL");
	...
	public function saveRadHeader($kd,$nm) {
	...
	public function hapusRadHeader($id) {
		$this->db->select("id");
		$this->db->from("ptindakan_instalasi");
		$this->db->where("notransaksi_IN", $id);
		$data = $this->db->get();
	...
	public function ambildatatindakanrad_rme() {
		$this->db->select("kode_tindakan, tindakan, jasas");
		$this->db->from("mtindakan");
	...
	public function saveItemRad($kd,$nm) {
		$date = date_create($this->input->get("radtgl"));
		$tgl = date_format($date,"Y-m-d");
		$datasimpan = array(
			'kode_unit' => $kd, 
...
public function saveEditRadHeader($notransaksi_IN) {
	$dataubah = array(
		'tanggal' =>  $this->input->get("tanggalorder"), 
		'kode_dokter' => $this->input->get("dokterf"), 
		'nama_dokter' => $this->input->get("dokterftext"), 
