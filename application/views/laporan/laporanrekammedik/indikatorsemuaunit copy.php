 <html>
	...
	<body>
        ...
        // $sql="SELECT tgl_masuk,tgl_keluar,saat_keluar,kondisi_keluar keluarpada from register where (bagian='INAP' or bagian='IGD') and MONTH(tgl_keluar)='$bulannya' and YEAR(tgl_keluar)='$tahunnya' and status=1 and kode_keluarpada<>'$keluarigd' and kode_keluarpada<>'$keluarigdp' and kode_keluarpada<>'$keluarkmb' order by tgl_keluar ";
		
		$sql="SELECT tgl_masuk,tgl_keluar,saat_keluar,kondisi_keluar,keluarpada from register where (bagian='INAP' or bagian='IGD') and MONTH(tgl_keluar)='11' and YEAR(tgl_keluar)='2022' and status=1 and kode_keluarpada<>'IGDD' and kode_keluarpada<>'IGDP' and kode_keluarpada<>'KMBS' order by tgl_keluar ";


		$qry2=$this->db->query($sql);
		foreach ($qry2->result_array() as $d) {
			$nhari=$d['tgl_keluar']-$d['tgl_masuk'];
			if ($nhari == 0) { $nhari = 1; }
        ...
        $sql="SELECT tgl_masuk,tgl_keluar,saat_keluar,kondisi_keluar,keluarpada from register where (bagian='INAP' or bagian='IGD') and MONTH(tgl_keluar)='$bulannya' and YEAR(tgl_keluar)='$tahunnya' and MONTH(tgl_masuk)='$bulannya' and YEAR(tgl_masuk)='$tahunnya' and pulang>0 and kode_keluarpada<>'$keluarigd' and kode_keluarpada<>'$keluarkmb' ";
		$qry2=$this->db->query($sql);
		foreach ($qry2->result_array() as $d) {
        ...
        $sql="SELECT tgl_masuk,tgl_keluar,saat_keluar,kondisi_keluar, keluarpada from register where (bagian='INAP' or bagian='IGD') and MONTH(tgl_masuk)='$bulannya' and YEAR(tgl_masuk)='$tahunnya' and pulang=0 ";
		$qry2=$this->db->query($sql);
		foreach ($qry2->result_array() as $d) {
                $tglm=$d['tgl_masuk'];
        ...
        $sql="SELECT tgl_keluar,saat_keluar,kondisi_keluar, keluarpada from register where (bagian='INAP' or bagian='IGD') and MONTH(tgl_keluar)='$bulannya' and YEAR(tgl_keluar)='$tahunnya' and MONTH(tgl_masuk)<>'$bulannya' and YEAR(tgl_masuk)<>'$tahunnya' and pulang>0 and kode_keluarpada<>'$keluarigd' and kode_keluarpada<>'$keluarkmb' ";
		$qry2=$this->db->query($sql);
		foreach ($qry2->result_array() as $d) {
                $tglk=$d['tgl_keluar'];
