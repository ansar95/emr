    ...
    <td><table width="100%" border="1" cellspacing="0">
      ...
      <?php
            ...
            $xtottidaktepatwaktu=0;
             $qry1=$this->db->query("SELECT golongan FROM register where tglsetor>='".$tglawal."' and tglsetor<='".$tglakhir."' and bagian='INAP' group by golongan order by golongan ");
            foreach ($qry1->result_array() as $brs1) {
              $xnomor++;
              $xgolongan=$brs1['golongan'];  
              $qry2=$this->db->query("SELECT golongan,lengkap,waktusetor FROM register where tglsetor>='".$tglawal."' and tglsetor<='".$tglakhir."' and bagian='INAP' and golongan='".$xgolongan."'order by golongan");
