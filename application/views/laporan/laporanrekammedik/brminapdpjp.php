    ...
    <td><table width="100%" border="1" cellspacing="0">
      ...
      <?php
            ...
            $xtottidaktepatwaktu=0;
             $qry1=$this->db->query("SELECT kode_dokter FROM register where tglsetor>='".$tglawal."' and tglsetor<='".$tglakhir."' and bagian='INAP' group by kode_dokter order by kode_dokter ");
            foreach ($qry1->result_array() as $brs1) {
              $xnomor++;
              $xkode_dokter=$brs1['kode_dokter'];  
              $qry2=$this->db->query("SELECT nama_dokter,lengkap,waktusetor FROM register where tglsetor>='".$tglawal."' and tglsetor<='".$tglakhir."' and bagian='INAP' and kode_dokter='".$xkode_dokter."'order by kode_dokter");
