...
<body>
		...
		// $qry2=$this->db->query("SELECT pgizi.no_rm as no_rm, pgizi.pagi as pagi, pgizi.siang as siang, pgizi.malam as malam, pgizi.bentuk as bentuk, pasien.nama_pasien as nama_pasien, pasien.tgl_lahir as tgl_lahir, mkamar.nama_kamar as nama_kamar, mkamar.nama_kelas as nama_kelas FROM pgizi left join pasien on pgizi.no_rm=pasien.no_rm  left join mkamar on pgizi.kode_kamar=mkamar.kode_kamar where pgizi.id = '$id' ");

		$qry2=$this->db->query("SELECT pgizi.no_rm as no_rm, pgizi.pagi as pagi, pgizi.siang as siang, pgizi.malam as malam, pgizi.bentuk as bentuk, pasien.nama_pasien as nama_pasien, pasien.tgl_lahir as tgl_lahir, mkamar.nama_kamar as nama_kamar, mkamar.nama_kelas as nama_kelas, munit.nama_unit as nama_unit FROM pgizi left join pasien on pgizi.no_rm=pasien.no_rm  left join mkamar on pgizi.kode_kamar=mkamar.kode_kamar left join munit on pgizi.kode_unit=munit.kode_unit where pgizi.id = '$id' ");
