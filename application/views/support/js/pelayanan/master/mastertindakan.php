...
<script type="text/javascript">
	...
	function load_data(page) {
		...
		cari.addEventListener('click', function(){
			...
			console.log(key1);
                $.ajax({
                    url:"<?php echo site_url();?>/masterdata/paginationtindakan/"+"1",
                    method:"GET",
                    data: {key1: key1},
                    dataType:"json",
                    success:function(data) {
                        $('#tabletindakan').html(data.country_table);
                        $('#pagination_link').html(data.pagination_link);
                    }
                });
        })       
        $.ajax({
                    url:"<?php echo site_url();?>/masterdata/paginationtindakan/"+page,
                    method:"GET",
                    data: {key1: key1},
                    dataType:"json",
                    success:function(data) {
	...
	$(document).ready(function () {
		...
		$("#tambahtindakan").click(function(e) {
			modaltable();
			$.ajax({
				url: "<?php echo site_url();?>/masterdata/panggiltambahtindakan",
				type: "GET",
    ...
    function panggiledit(id) {
        modaltable();
        $.ajax({
            url: "<?php echo site_url();?>/masterdata/panggiledittindakan",
            type: "GET",
            data: {id: id},
            success: function (ajaxData){
                $("#formmodal").html(ajaxData);
        ...
        $.confirm({
            ...
            buttons: {
                ...
                hapus: {
                    ...
                    action: function(){
                        $.ajax({
                            url: "<?php echo site_url();?>/masterdata/deletetindakan",
                            type: "GET",
                            data : {
                                id: id
                            },
