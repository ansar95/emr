...
<div class="modal fade" id="formmodal" data-backdrop="static" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-keyboard="false">
    ...
    <script type="text/javascript">
        ...
        function modaldetail() {
            $("#kotakdetail").append('<div class="overlay text-center"><i class="fa fa-lock h4 m-5" aria-hidden="true"></i></div>');
        }

        function modaldetailtutup() {
            $("#kotakdetail").find('.overlay').remove();
        }
        ...
        function statuspasien(tipe) {
            if (tipe.value == "Umum") {
                ...
                $("#sjp").prop("disabled", false);
                $("#namagolongan").val('UMUM');
                $.ajax({
        ...
        function carirm() {
            ...
            $.ajax({
                ...
                $("#namagolongan").val(t.dtpasien[0].golongan);
                if (tipe == "Umum") {
                    t.dtunit.forEach(function(entry) {
                        var option = new Option(entry.nama_unit, entry.kode_unit, false, false);
                        $('#unit').append(option).trigger('change');
        ...
        function prosesheader() {
            ...
            var racik = $("#racik").val();
            var nonracik = $("#nonracik").val();
            if ((norm == "") || (nama == "") || (namagolongan == "") || (tglresep == "") || (unit == "") || (dokter == "")) {
                kuranglengkap();
                return;
            }
            $("#norm").prop("disabled", true);
            ...
            $.confirm({
                ...
                buttons: {
                    hapus: {
                        ...
                        action: function() {
                            $.ajax({
                                url: "<?php echo site_url(); ?>/depoapotik/prosescekresep",
                                type: "GET",
                                data: {
                                    id: id,
                                    norep: norep
        ...
        // ================= cetak etiket 
        function cetaketiketobat(id) {
            var win = window.open("<?php echo site_url(); ?>/sampul/cetaketiket/" + id, '_blank');
            win.focus();
        }

        function cetaketiketsemua() {
            var norep = document.getElementById("norep").value;
            var win = window.open("<?php echo site_url(); ?>/sampul/cetaketiketsemua/" + norep, '_blank');
            win.focus();
        }
    </script>