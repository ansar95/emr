...
<div class="modal fade" id="formmodal" data-backdrop="static" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-keyboard="false">
    ...
    <script type="text/javascript">
        ...
        function prosescarirt() {
            ...
            $.ajax({
                url: "<?php echo site_url();?>/fakturpelayanan_rt/cariunitbydate",
                type: "GET",
                data: {tgl: tgl},
        ...
        function prosesambildetail(kode, nama, ampra) {
            ...
            $.ajax({
                url: "<?php echo site_url();?>/fakturpelayanan_rt/ambildetailunit",
                type: "GET",
        ...
        function panggildrop(id) {
            ...
            kodedrop = id;
            $.ajax({
                url: "<?php echo site_url();?>/fakturpelayanan_rt/tambahdetaildropping",
                type: "GET",
                data: {id: id},
                success: function (ajaxData){
        ...
        function panggiltambahdrop() {
            modaldetail();
            $.ajax({
                url: "<?php echo site_url();?>/fakturpelayanan_rt/panggiltambahdetaildropping",
                type: "GET",
                // data: {id: id},
                success: function (ajaxData){
                    $("#formmodal").html(ajaxData);
        ...
        function cetakdroping($xx) {   
            // var ruang = document.getElementById("unitname").value;
            var ruang = $xx;
		    var tgl = document.getElementById("tgl").value;
		    var tglx= tgl.substr(6, 4)+'-'+tgl.substr(0, 2)+'-'+tgl.substr(3, 2) 
            var win = window.open("<?php echo site_url();?>/fakturpelayanan/cetaklembarampra/" + ruang +"/"+ tglx , '_blank');
		    win.focus();
        }
            ...
            $.confirm({
                ...
                buttons: {
                    hapus: {
                        ...
                        action: function(){
                            $.ajax({
                                url: "<?php echo site_url();?>/fakturpelayanan_rt/hapusdropping",
                                type: "GET",
                                data: {
                                    id: id,
                                    kode: kodeunit,
