...
class Sampul extends CI_Controller {
	...
	public function gelangunitugd($id) {
		if ($this->session->userdata("login") == TRUE) {
			$r = date("Ymd");
			$this->load->library('pdf');
			$this->pdf->setPaper('gelang', 'landscape');
...
// ========== RESEP ETIKET ================
	...
	public function cetaketiket($id) {
		if ($this->session->userdata("login") == TRUE) {
			$r = date("Ymd");
			$this->load->library('pdf');
			$this->pdf->setPaper('etiket', 'landscape');
...
// ========== GIZI ETIKET ================
	public function cetaketiketpagi($id) {
		if ($this->session->userdata("login") == TRUE) {
			...
			// $dompdf = new Dompdf();
			// $customPaper = array(0, 0, 65, 40); // Lebar dan tinggi dalam mm
	...
	// billing============
	public function billinginap_old($id) {
		if ($this->session->userdata("login") == TRUE) {
			$r = date("Ymd");
			$this->load->library('pdf');
			$this->pdf->setPaper('A4', 'potrait');
